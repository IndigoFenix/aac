...
export function CommuniAACteFeature() {
  ...
  const imageInterpretMutation = useMutation({
    mutationFn: async ({ file, context }: { file: File; context: any }) => {
      ...
      const res = await apiRequest('POST', `/api/interpret`, formData);
  ...
  const redeemInviteCodeMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: `${t("label.aacUser")} "${data.studentName}" ${language === "he" ? "נוסף בהצלחה" : "has been added successfully"}`,
      });
    },
    onError: (error: any) => {
  ...
  const handleContextSubmit = async () => {
    ...
    const aacUserContext = selectedAacUser
      ? [
          `${t("label.aacUser")}: ${selectedAacUser.name}`,
    ...
    if (pendingInterpretation === "text") {
      ...
      const interpretationRequest = {
        ...
        inputType: "text",
        language,
        studentId: selectedAacUser?.id || null,
        studentName: selectedAacUser?.name || null,
...
Generated by CommuniAACte`;
  ...
  return (
    <div dir={isRTL ? "rtl" : "ltr"}>
      {/* === A) MAIN FEATURE CONTENT (paste from <main>...</main>) === */}
      <div className="max-w-4xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
        {/* 1) ⬇⬇ Paste EXACTLY the block labeled "Input Section" ⬇⬇ */}
        {/* Input Section */}
                    ...
                    {historyData.interpretations.map(
                      (interpretation: Interpretation) => (
                        <div
                          ...
                          onClick={() => handleSelectHistoryItem(interpretation)}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleSelectHistoryItem(interpretation); }
                          }}
                        >
                          ...
                          <div className="text-xs sm:text-sm space-y-1">
                            ...
                            interpretation.imageData ? (
                              <div className="flex items-center gap-2">
                                ...
                                {/* Image-as-lightbox-trigger nested inside an
                                    already-clickable card. Replacing with a
                                    real button creates a nested-button
                                    antipattern. The parent row has its own
                                    keyboard handler; this is a pointer-only
                                    convenience. */}
      ...
      {/* === B) DIALOGS & POPUPS (paste these AFTER the main content) === */}
                  ...
                  {aacUsersLoading ? (
                    ...
                    aacUsersData.aacUsers
                      .filter(
                        (aacUser: any) =>
                          aacUser.studentId &&
                          String(aacUser.studentId).trim() !== "",
                      )
                      .map((aacUser: any) => (
                        <SelectItem
                          key={aacUser.studentId}
                          value={String(aacUser.studentId)}
                        >
                          <div className="flex flex-col">
                            <span className="font-medium">{aacUser.name}</span>
                            <span className="text-xs text-muted-foreground">
                              {[
                                aacUser.age &&
                  ...
                  <Form {...createInviteForm}>
                    <form
                      onSubmit={createInviteForm.handleSubmit(
                        handleCreateInviteCode as any,
                      )}
                      className="space-y-4"
                    >
                      <FormField
                        control={createInviteForm.control as any}
                        name="studentId"
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>
      ...
      {/* 9) Settings Dialog */}            {/* <-- paste full <Dialog open={showSettingsDialog} ...> block */}
      {/* 10) Privacy Policy Dialog */}     {/* <-- paste full <Dialog open={showPrivacyDialog} ...> block */}
      {/* 11) About Dialog */}              {/* <-- paste full <Dialog open={showAboutDialog} ...> block */}
      {/* 12) Terms of Service Dialog */}   {/* <-- paste full <Dialog open={showTermsDialog} ...> block */}
      {/* 13) Schedule Manager (conditional) */} {/* <-- paste the {scheduleAacUser && <ScheduleManager ... />} */}
    </div>
  );
}
