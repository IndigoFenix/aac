...
export interface VariationDimension {
  ...
  /**
   * APPEARANCE ONLY — this dimension changes how a thing LOOKS and has no
   * spoken word and no board icon. The language layer skips it in the adjective
   * walk and the compositor skips it in the badge stack; without that a facet
   * with no lexeme gets spelled out from its raw glyph ("toy material wood
   * car") and a facet with no art draws a broken badge.
   *
   * Colour is deliberately NOT appearance-only: "the red ball" is a thing a
   * child says, and `color_*` has words in every locale and art on the board.
   * The test for this flag is "would anyone SAY it", not "does it change
   * pixels" — a dimension graduates out of it by gaining lexemes and an icon.
   */
...
export const MATERIAL_DIMENSION: VariationDimension = {
  key: "material",
  appearanceOnly: true, // no lexemes, no icon — it makes a wooden ball look wooden
...
export const FORM_DIMENSION: VariationDimension = {
  ...
  values: ["toy"],
  look: {
    // 0.75, not the 0.4 this shipped with. Loose props all spawn at a uniform
    // ~0.35 m radius, so 0.4 made a toy barely a third the size of every other
    // item in the room and it read as a speck on the floor. A doll should be
    // visibly SMALLER THAN THE REAL THING while still being a thing you can see
    // and want — that is a gentle reduction, not a miniature.
    toy: { scale: [0.75, 0.75, 0.75], roughness: 0.9, metalness: 0 },
  },
};
...
/** Facet values belonging to an `appearanceOnly` dimension — never spoken,
 *  never badged. The ONE definition both the language layer and the board
 *  compositor read, so the two can't drift on which facets are wordless. */
export const APPEARANCE_ONLY_VALUES: ReadonlySet<string> = new Set(
  Object.values(VARIATION_DIMENSIONS)
    .filter((d) => d.appearanceOnly)
    .flatMap((d) => d.values),
);

/** Does this facet only affect how a thing LOOKS (no word, no icon)? */
export const isAppearanceOnlyFacet = (facet: string): boolean =>
  APPEARANCE_ONLY_VALUES.has(facet);
