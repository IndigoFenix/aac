...
function furnishPlan(
  ...
  for (const def of defs) {
    ...
    if (rule.mode === "besideAnchor") {
      ...
      const z = zoneForCell(ctx, def.cell);
      // A MOUNTED SEAT (a chair — the mount-room rule, placement.ts) must
      // also leave a body room to stand and sit down: a side that fits the
      // seat's own 0.22 m footprint but has less than a body's radius
      // between the seat and the wall is unfit — the observed defect was a
      // chair 4 cm short of standable behind, whose sitter therefore parked
      // across the table.
      const seatOk = (c: { u: number; v: number }): boolean =>
        !mountedSeatKind(def.kind) || seatMountable(ctx, z, c.u, c.v);
      if (rule.spread === "eachSide") {
        // Both sides independently, PERPENDICULAR to the door axis (the
        // door→anchor lane stays open). A blocked side may retry the OTHER
        // axis when the rule allows it — a door corridor claiming one flank
        // (the partitioned case this retry was built for) and a wall
        // stealing the flank's mount room are the same shape of problem —
        // omitted only when both are taken, never squeezed.
        if (!anchor) continue;
        const gap = anchor.piece.radius + def.radius + rule.gapPad;
        const spotOn = (ax: "u" | "v", s: number): { u: number; v: number } =>
          ax === "u" ? { u: anchor.u + s * gap, v: anchor.v } : { u: anchor.u, v: anchor.v + s * gap };
        const otherAxis = rule.axis === "u" ? "v" : "u";
        ([-1, 1] as const).forEach((side, i) => {
          // The piece's own side of the main axis first; with the retry flag,
          // ANY free side of the anchor — the perpendicular pair, then the
          // mirrored main side (fits' piece-overlap keeps a sibling's spot
          // taken). A table wants its seats wherever bodies can actually sit.
          const tries = rule.retryOtherAxisWhenPartitioned
            ? [spotOn(rule.axis, side), spotOn(otherAxis, side), spotOn(otherAxis, -side), spotOn(rule.axis, -side)]
            : [spotOn(rule.axis, side)];
          const at = tries.find((c) => fitsSvc(ctx, z, c.u, c.v, def.radius, svc) && seatOk(c)) ?? null;
          if (!at) return;
      ...
      const near = anchor
        ? ([-1, 1] as const)
            ...
            .find((c) => fitsSvc(ctx, z, c.u, c.v, def.radius, svc) && seatOk(c))
        : undefined;
      const at = near ?? (rule.fallbackWalls ? scanWalls(ctx, z, rule.fallbackWalls, def.radius, svc) : null);
      if (at && seatOk(at)) push(z, idOf(def.key), def.kind, at, def.radius, def.openable);
      continue;
    }
    ...
    for (const t of targets) {
      ...
      // A wall-flush MOUNTED SEAT can never be sat on (the wall band covers
      // its mounting spot) — an authored chair on a wallScan rule goes
      // without rather than standing as scenery (the mount-room rule).
      if (mountedSeatKind(def.kind) && !seatMountable(ctx, t.z, at.u, at.v)) continue;
