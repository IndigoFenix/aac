...
import { useContractFor } from "../../furniture-use";
...
export function fits(ctx: PlacementContext, z: Zone, u: number, v: number, r: number): boolean {
  ...
  // A seated body's MOUNT SPOT is reserved ground exactly like a shopper's
  // standing spot: a chair checks its own mount room when IT is placed (the
  // mount-room rule below), but the cupboard placed after it must not take
  // that ground back — the seat would silently become scenery.
  for (const s of seatMountSpotsIn(ctx, z)) {
    if (Math.abs(u - s.u) < r + berth && Math.abs(v - s.v) < r + berth) return false;
  }
  return true;
}

// ---- THE MOUNT-ROOM RULE: a SEAT is only placeable where a body can STAND
// to get onto it. A pass-through piece a body POSES ON (the use contract's
// `onFixture` — a chair; a bowl is used from above and never mounted) is
// worthless if no body can reach its mounting spot: the observed defect was a
// chair legally tucked between the table and the east wall with 4 cm less
// than a body's radius behind it — planning (probing with tangency slack)
// marginally accepted the spot, locomotion (the true radius) could never
// deliver it, and the sitter ended up parked across the table. `fits` judges
// only the piece's OWN footprint (0.22 m), so it can't see this; these
// predicates judge the BODY's.
//
// The mount spot mirrors the interaction layer's stand-point resolution
// ("THE SEAT CHOOSES THE SIDE", stand-points.ts): the seat's own centre when
// it is standable, else — when a solid piece's keep-out covers the seat (a
// chair tucked under its table) — the stand-off spot on the seat's outward
// face, the dominant axis from the covering piece through the seat. The two
// layers share the arithmetic (radius + bodyR + MOUNT_GAP) so the generator
// never emits a seat the walker's own resolution can't approach.

/** The comfortable gap between a covering piece's keep-out and the mounting
 *  body — stand-points.ts's 0.22 stand-off, kept in sync by value. */
const MOUNT_GAP = 0.22;

/** Is `kind` a seat a body MOUNTS (poses on) rather than merely stands
 *  beside? Derived from the use contract — pass-through + onFixture — so a
 *  future pass-through stool inherits the rule with no new code. */
export function mountedSeatKind(kind: StationKind): boolean {
  return PASS_THROUGH.has(kind) && useContractFor(kind).onFixture;
}

/** Can a body of the DESIGN radius stand at frame (u,v)? Walls keep the
 *  svcWall band (opened inside door corridors — a doorway is passage, the
 *  service flood's own rule) and solid pieces keep radius + bodyR, at the
 *  FULL body radius: placement must be conservative where the planner may
 *  shave (locomotion enforces the true radius, so a generator-legal spot
 *  must clear it outright). */
export function bodyStandable(ctx: PlacementContext, z: Zone, u: number, v: number): boolean {
  const wallBand =
    u < z.u0 + ctx.svcWall || u > z.u1 - ctx.svcWall || v < z.v0 + ctx.svcWall || v > z.v1 - ctx.svcWall;
  if (wallBand && !z.corridors.some((c) => u > c.u0 && u < c.u1 && v > c.v0 && v < c.v1)) return false;
  for (const p of ctx.pieces) {
    if (PASS_THROUGH.has(p.kind)) continue;
    const q = toFrame(ctx, p.x, p.y);
    if (Math.abs(u - q.u) < p.radius + ctx.bodyR && Math.abs(v - q.v) < p.radius + ctx.bodyR) return false;
  }
  return true;
}

/** The mount spot for a seat at frame (u,v): the seat's own centre when a
 *  body can stand there, else the stand-off on the seat's face — the
 *  dominant axis from the covering solid piece through the seat. Null ⇒ no
 *  body can ever get onto it. */
function seatMountSpot(
  ctx: PlacementContext,
  z: Zone,
  u: number,
  v: number,
): { u: number; v: number } | null {
  if (bodyStandable(ctx, z, u, v)) return { u, v };
  // The solid piece whose keep-out covers the seat (the tucked-under table).
  for (const p of ctx.pieces) {
    if (PASS_THROUGH.has(p.kind)) continue;
    const q = toFrame(ctx, p.x, p.y);
    if (Math.abs(u - q.u) > p.radius + ctx.bodyR || Math.abs(v - q.v) > p.radius + ctx.bodyR) continue;
    const du = u - q.u;
    const dv = v - q.v;
    const stand = p.radius + ctx.bodyR + MOUNT_GAP;
    const d =
      Math.abs(du) >= Math.abs(dv)
        ? { u: Math.sign(du) || 1, v: 0 }
        : { u: 0, v: Math.sign(dv) || 1 };
    const s = { u: q.u + d.u * stand, v: q.v + d.v * stand };
    return bodyStandable(ctx, z, s.u, s.v) ? s : null;
  }
  return null; // unstandable with no covering piece (a wall-flush seat)
}

/** THE MOUNT-ROOM RULE for a seat candidate at frame (u,v): its centre is
 *  standable, or the covering solid piece leaves a standable stand-off on
 *  the seat's own face. False ⇒ the seat would be furniture nobody can sit
 *  on — unplaceable, exactly like not fitting. */
export function seatMountable(ctx: PlacementContext, z: Zone, u: number, v: number): boolean {
  return seatMountSpot(ctx, z, u, v) !== null;
}

/** The live mount spots of every mounted seat standing in `z` — each is
 *  reserved ground (fits / placementFeasible keep later pieces a body's
 *  berth off it), derived fresh from the piece list so every caller sees
 *  the same reservations with no bookkeeping. */
function seatMountSpotsIn(ctx: PlacementContext, z: Zone): Array<{ u: number; v: number }> {
  const out: Array<{ u: number; v: number }> = [];
  for (const p of ctx.pieces) {
    if (!mountedSeatKind(p.kind)) continue;
    const f = toFrame(ctx, p.x, p.y);
    if (f.u < z.u0 || f.u > z.u1 || f.v < z.v0 || f.v > z.v1) continue;
    const s = seatMountSpot(ctx, z, f.u, f.v);
    if (s) out.push(s);
  }
  return out;
}
...
export function serviceOk(ctx: PlacementContext, z: Zone, cu: number, cv: number, cr: number): boolean {
  ...
  // A diner's MOUNT SPOT is a service spot too (the mount-room rule): the
  // candidate must leave it flood-REACHED, not merely uncovered — observed:
  // a cupboard displaced into the last south-wall pocket kept clear OF the
  // mount spot yet sealed the whole strip the room's chairs mount from,
  // leaving the spot standable on an island no walker could enter.
  for (const sp of seatMountSpotsIn(ctx, z)) {
    if (sp.u < z.u0 || sp.u > z.u1 || sp.v < z.v0 || sp.v > z.v1) continue;
    const iu = Math.min(nu - 1, Math.max(0, Math.floor((sp.u - z.u0) / du)));
    const iv = Math.min(nv - 1, Math.max(0, Math.floor((sp.v - z.v0) / dv)));
    if (state[iv * nu + iu] !== 3) return false;
...
export function placementFeasible(
  ...
  // A seated diner's mount spot is reserved standing ground just like a
  // shopper's (fits' own reservation, mirrored for parity).
  {
    const berth = Math.max(SPOT_CLEAR, ctx.bodyR + 0.1);
    for (const s of seatMountSpotsIn(ctx, z)) {
      if (Math.abs(u - s.u) < r + berth - F_EPS && Math.abs(v - s.v) < r + berth - F_EPS) {
        return { ok: false, reason: "goods-spot" };
      }
    }
  }
  ...
  // A seat nobody could stand to sit on is as impossible as one that pinches
  // the room — the same "no room for a body there" refusal (THE MOUNT-ROOM
  // RULE above; this is what kept crafted chairs off walls and out of the
  // dead strip between a table and its wall).
  if (mountedSeatKind(cand.kind) && !seatMountable(ctx, z, u, v)) {
    return { ok: false, reason: "service" };
  }
  return { ok: true };
}
...
/**
 * HOW HARD the named anchor binds — the spoken proximity distinction ("put the
 * chair NEXT TO the table" vs "NEAR the table"), which the registry already
 * drew for itself with `besideAnchor`.
 *
 *   • `beside` — the ADJACENT band only: the snug ring right against the
 *     anchor's footprint (plus a body's slack). A wall spot across the room is
 *     not "next to" anything, however well it scores.
 *   • `near` — the anchor's VICINITY, sized by the room rather than by a fixed
 *     gap: every feasible spot within reach survives, and each one's score is
 *     multiplied by how close it got, so the nearest good spot wins.
 *
 * Absent ⇒ the legacy undirected search: the anchor only SEEDS extra candidates
 * and competes on the station's own aesthetics.
 */
export type AnchorMode = "beside" | "near" | "front" | "behind";

/** Slack past the snug ring that still counts as ADJACENT — half a body, the
 *  same tolerance the candidate de-duplication uses. */
const BESIDE_SLACK = 0.5;
...
export interface PlacementQuery {
  ...
  anchorGap?: number;
  /** How the anchor CONSTRAINS the search (the spoken relation). */
  anchorMode?: AnchorMode;
  /** Which way the anchor piece FACES (world radians, 0 = +x) — required for
   *  the directional modes (`front`/`behind`); without it they degrade to
   *  `near`, since a bare point has no front. */
  anchorFacing?: number;
}
...
export function placementCandidates(
  ...
  // AN ANCHORED KIND SUPPLIES ITS OWN ANCHOR: a chair's habitat is beside
  // its table (the registry's besideAnchor rule), not along the walls — but
  // an undirected search ("put + chair", no spot named) used to generate
  // only the wall/corner candidates below, and the besideAnchor SCORE then
  // dragged the winner to the wall nearest the table: crafted chairs piled
  // up flush on that wall, behind the seated ones. When the caller names no
  // anchor, resolve the rule's own — the nearest placed piece of the anchor
  // kind — so the ring candidates around it exist to be scored at all.
  let anchor = q.anchor;
  const placeRule = def.place;
  if (!anchor && placeRule.mode === "besideAnchor") {
    const anchorDef = DEFAULT_WORKSTATION_REGISTRY.byKey(placeRule.anchor);
    const zc = toWorld(ctx, (zone.u0 + zone.u1) / 2, (zone.v0 + zone.v1) / 2);
    const hit = ctx.pieces
      .filter((p) => anchorDef && p.kind === anchorDef.kind)
      .reduce<FurniturePiece | null>(
        (best, p) =>
          !best || Math.hypot(p.x - zc.x, p.y - zc.y) < Math.hypot(best.x - zc.x, best.y - zc.y) ? p : best,
        null,
      );
    if (hit) anchor = { x: hit.x, y: hit.y };
  }

  // Rings around the anchor first (the directed spot is the point). The snug
  // ring must clear whatever solid piece the anchor point sits ON: "near the
  // table" anchors at the table's CENTRE, and the old r + 0.35 default put
  // the whole first ring (and most of the second) inside the tabletop — every
  // candidate infeasible, only the far ring survived.
  let snugRing = 0;
  if (anchor) {
    const at = anchor;
    const a = toFrame(ctx, at.x, at.y);
    const covering = ctx.pieces.find(
      (p) =>
        !PASS_THROUGH.has(p.kind) && Math.abs(p.x - at.x) <= p.radius && Math.abs(p.y - at.y) <= p.radius,
    );
    const snug = q.anchorGap ?? (covering ? covering.radius + r + 0.1 : r + 0.35);
    snugRing = snug;
    for (const ring of [snug, snug + 0.4, snug + 0.9]) {
      for (let i = 0; i < 12; i++) {
        ...
        consider(u, v, Math.atan2(at.y - toWorld(ctx, u, v).y, at.x - toWorld(ctx, u, v).x));
      }
    }
  }
  ...
  // THE SPOKEN RELATION DECIDES WHICH OF THESE STILL COUNT. Without a mode the
  // anchor merely SEEDS candidates and the wall spots compete on the station's
  // own aesthetics — which is how "put the chair near the table" ended up
  // against a wall on the far side of the room.
  const ranked = anchor && q.anchorMode
    ? constrainToAnchor(out, anchor, q.anchorMode, snugRing, zone, q.anchorFacing)
    : out;
  ranked.sort((a, b) => b.score - a.score);
  return ranked.slice(0, max);
}

/** A directional spot must sit within this wedge of the anchor's facing (or
 *  its opposite, for `behind`) — generous enough that a ring candidate one
 *  step off-axis still counts, strict enough that "in front" never lands at
 *  the side. */
const DIRECTIONAL_WEDGE_COS = Math.cos((75 * Math.PI) / 180);

/** Apply an `AnchorMode` to a finished candidate list: `beside` KEEPS only the
 *  adjacent band; `near` keeps the anchor's room-scaled vicinity and multiplies
 *  each score by its closeness, so distance ranks rather than merely admits.
 *  `front`/`behind` keep the near-band spots inside the facing (or blind-side)
 *  wedge, ranked by closeness AND alignment; with no facing supplied they
 *  degrade to `near` — a bare point has no front. */
function constrainToAnchor(
  ...
  const dOf = (s: RatedSpot) => Math.hypot(s.x - anchor.x, s.y - anchor.y);
  if (mode === "beside") return spots.filter((s) => dOf(s) <= snug + BESIDE_SLACK);
  // "Near" SCALES: the acceptance radius is half the room's diagonal (never
  // smaller than the outermost ring the search generated), so a big hall is
  // forgiving and a closet is strict — never a hard-coded metre.
  const reach = Math.max(snug + 0.9, Math.hypot(zone.u1 - zone.u0, zone.v1 - zone.v0) / 2);
  const nearRanked = spots
    .filter((s) => dOf(s) <= reach)
    .map((s) => ({ ...s, score: s.score * (1 - 0.75 * (dOf(s) / reach)) }));
  if (mode === "near" || facing === undefined) return nearRanked;
  // Directional: the axis is the anchor's facing (flipped for `behind`).
  const ax = Math.cos(facing) * (mode === "behind" ? -1 : 1);
  const ay = Math.sin(facing) * (mode === "behind" ? -1 : 1);
  return nearRanked
    .map((s) => {
      const d = dOf(s);
      if (d < 1e-6) return null; // the anchor point itself has no direction
      const align = ((s.x - anchor.x) * ax + (s.y - anchor.y) * ay) / d;
      if (align < DIRECTIONAL_WEDGE_COS) return null;
      return { ...s, score: s.score * align };
    })
    .filter((s): s is RatedSpot => s !== null);
}
