...
export function createSpiritLadder(opts: SpiritLadderOpts): SpiritLadder {
  ...
  function stepTown(pointer: SpiritPointer | null, dt: number, now: number): SpiritStepResult {
    ...
    if (pointer && !t.exiting) {
      const ndcX = (pointer.x / w) * 2 - 1;
      const ndcY = -((pointer.y / h) * 2 - 1);
      const inZone = t.blend > 0.9 && !inOrbitBand(ndcX) && ndcY < 0.9;
      // CORNERS circle the focus — the same law at every framing (see
      // corner-orbit.ts): the vertical half picks the direction.
      t.targetAz += cornerOrbitDelta(ndcX, ndcY, dt);

      // The bottom EXIT strip is the bottom CENTRE only: the lower corners
      // belong to the orbit, and a corner that also accrued the exit dwell
      // would climb a rung out from under the player mid-circle.
      if (ndcY < -0.7 && !inOrbitBand(ndcX)) {
  ...
  function stepStructure(pointer: SpiritPointer | null, dt: number, now: number): SpiritStepResult {
    ...
    // Gaze: the screen CORNERS orbit the dollhouse azimuth; bottom-dwell
    // (bottom centre, clear of the orbit bands) steps back out;
    // pointer forwards to the host (interaction) only once the blend has
    // essentially arrived.
    if (pointer) {
      const ndcX = (pointer.x / w) * 2 - 1;
      const ndcY = -((pointer.y / h) * 2 - 1);
      s.spiritAz += cornerOrbitDelta(ndcX, ndcY, dt);
