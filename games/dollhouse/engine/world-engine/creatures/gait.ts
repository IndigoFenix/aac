...
export function bodyBob(p: GaitParams, legCount: number): BodyBob {
  ...
  // legs sharing the support. A running gait (duty < ~0.5, flight phases)
  // bounces harder — the ballistic hop between stances.
  const run = 1 + 2.5 * Math.max(0, 0.55 - p.dutyFactor);
  const amp = (0.03 * run) / Math.max(1, legCount * 0.5);
  return { dy: amp * (0.5 - 0.5 * Math.cos(p.phase * Math.PI * 4)) };
}

// ── Gait selection — walk ↔ run from one speed dial ─────────────────────
// Speed 0..1 picks every cycle parameter on a continuum: a slow walk keeps
// a foot down (duty > 0.5, short strides), a full run commits to flight
// phases (duty < 0.5, long strides, high lift). Cadence follows leg length
// (long legs cycle slower — pendulum scaling), so the same dial works for
// any creature.
...
export interface LocomotionParams {
  ...
  stepHeight: number;
  dutyFactor: number;
  /** Gait cycles per second at this speed for this leg length. */
  cadenceHz: number;
  /** Rough ground speed (m/s) the cycle implies — stance sweep per time. */
  speedMps: number;
}

export function locomotionGait(speed01: number, legLenM: number): LocomotionParams {
  const s = Math.max(0, Math.min(1, speed01));
  const dutyFactor = 0.68 - 0.34 * s;
  const strideFrac = 0.3 + 0.5 * s;
  const stepHeight = 0.1 + 0.22 * s;
  const cadenceHz = (0.55 + 1.05 * s) * Math.sqrt(1 / Math.max(legLenM, 0.05));
  const speedMps = (strideFrac * legLenM * cadenceHz) / dutyFactor;
  return { strideFrac, stepHeight, dutyFactor, cadenceHz, speedMps };
}
