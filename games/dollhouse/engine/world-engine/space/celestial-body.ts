...
export interface CreateBodyOpts {
  ...
  compression?: number;
  /** SPIN multiplier (the world's `rotation` dial, scale.ts): this body turns
   *  that many times faster than its formation physics says. A world's day
   *  length IS its planet spinning faster — and the same factor reaches EVERY
   *  body in the universe, so relative day lengths are preserved. Default 1. */
  rotation?: number;
}
...
export function createCelestialBody(opts: CreateBodyOpts): CreatedBody {
  ...
  // The world's spin multiplier rides the body's OWN formation period, so a
  // slow Venus stays relatively slow — compression preserves ratios.
  const spinFactor = Math.max(1e-6, opts.rotation ?? 1);
  const rate = Math.abs(periodH) > 1e-6 ? ((2 * Math.PI) / (periodH * 3600)) * spinFactor : 0;
