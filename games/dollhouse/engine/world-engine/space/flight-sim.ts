...
import { PLAYER, CONTROLS, SPEED, WHEEL, BIRD_PULL } from "./flight-config.js";
import type {
  PlayerWorld, GravityContext, AtmosphericReading, CelestialBody,
} from "./world-types.js";
...
export interface Input {
  ...
  mouseX: number;
  mouseY: number;
  /** Mouse-wheel notches this frame (+ = faster, − = slower). The pilot's
   *  manual throttle on top of seagull's gaze-hyperdrive — an exponential
   *  speed factor applied in flight. Omit/0 for no change. */
  wheel?: number;
}
...
export interface PlayerState {
  ...
  hyperMult: number;
  /** Mouse-wheel throttle: an exponential factor scaling the flight base
   *  speed (the pilot's manual cruise control). 1 = seagull default. */
  wheelFactor: number;
...
export interface Player {
  ...
  /**
   * Install a hook that returns a unit world-space direction the avatar
   ...
   * toward it at BIRD_PULL per second. Used to face a focused body during
   * an approach (the object-readout overlay in seagull).
   */
  setReadoutHook(hook: (() => THREE.Vector3 | null) | null): void;
  /**
   * THE WALK→FLY HANDOFF (embedded-town flight). Launch straight into the
   * flying regime from an EXTERNALLY-supplied ground pose — the town host owns
   * walking, so when the player aims to lift off it seeds flight-sim here with
   * the avatar's real surface position and heading instead of running flight-
   * sim's own walker. `forward` is projected onto the local tangent; `speed`
   * defaults to the jump-takeoff speed. After this, drive the player with
   * `update()` each frame; watch `state.mode` flip back to "walking" for the
   * landing (hand control back to the town host there).
   */
  beginFlight(position: THREE.Vector3, forward: THREE.Vector3, speed?: number): void;
}

export interface CreatePlayerOpts {
  /** Whether the avatar can FLY. False = a plain ground walker: the
   *  top-of-screen takeoff zones are inert, wings never unfold, and the
   *  player stays in the walking regime. (There is no separate spaceship
   *  avatar — flight is this walker capability.) Default true. */
  canFly?: boolean;
}

export function createPlayer(world: PlayerWorld, opts: CreatePlayerOpts = {}): Player {
  const canFly = opts.canFly !== false;
  ...
  // With an ASYNC geology bake the home world may still be a smooth sphere
  // at spawn time — fall back to the sphere surface; the host re-places the
  // spawn when the terrain lands.
  const startSurface = home.surfaceAt
    ? home.surfaceAt(spawnAttempt, new THREE.Vector3())
    : new THREE.Vector3().copy(home.worldPosition).addScaledVector(startDir, home.radius);
  ...
  // beginFlight (walk→fly handoff) scratch.
  const _launchUp = new THREE.Vector3();
  const _launchSeed = new THREE.Vector3();
  ...
  function updateWalking(input: Input, dt: number, planet: NonNullable<GravityContext["dominant"]>) {
    ...
    const aimAboveGround = canFly && input.mouseY < runZoneCeiling && state.walkSpeed > 0;
  ...
  function updateAirborne(input: Input, dt: number, ctx: GravityContext) {
    ...
    if (readoutHook) {
      ...
      if (dir) {
        ...
        const k = Math.min(1, BIRD_PULL * dt);
    ...
    // MOUSE-WHEEL throttle: THE speed. The pilot's dialed factor times the
    // base cruise IS the target, constant at any altitude — no altitude
    // governor, no near-body auto-brake (both fought the pilot; user call).
    // The gaze hyperdrive still multiplies on top for interstellar legs,
    // and the low-altitude safety brake below still prevents pile-ins.
    if (input.wheel) {
      state.wheelFactor = Math.max(
        WHEEL.minFactor,
        Math.min(WHEEL.maxFactor, state.wheelFactor * Math.exp(WHEEL.gain * input.wheel)),
      );
    }
    const targetSpeed = SPEED.MIN_FLIGHT_SPEED * state.wheelFactor;
    ...
    if (state.lockedBodyId && state.lockProgress > 0) {
      ...
      if (tgt) {
        ...
        // The lock brake eats HYPERDRIVE speed on approach but never pushes
        // below the wheel's dialed speed — the wheel is the pilot's word.
        lockBlendedCap = Math.max(
          targetSpeed,
          Math.min(effectiveMaxSpeed, lockCap / state.lockProgress),
        );
    ...
    if (nearestBody && nearestBody.walkable) {
      // Obstacle-avoidance ring OFF (user call): the brake scales on plain
      // altitude instead of the 4-point terrain-clearance sweep — same
      // anti-crash friction, no per-frame ring sampling.
      obstacleClearance = nearestAlt;
      lowAltCap =
        PLAYER.flyLowAltCap *
    ...
    state.flightDebug.altSpeed = 0; // altitude governor removed — wheel is the speed
  ...
  //
  // The reflection is FLATTENED toward the surface: in flying mode nothing
  // ever pulls the nose back down (velocity carries only the planet frame,
  // and the altitude governor keeps feeding speed), so a skyward reflection
  // was a guaranteed one-way trip to space. A bounce skims instead.
  function bounceVelocity(s: PlayerState, retain: number) {
    ...
    // Clamp the up-component of the (possibly reflected) forward so the
    // rebound hugs the terrain instead of climbing away.
    const MAX_BOUNCE_UP = 0.15;
    const upDot = s.forward.dot(s.groundNormal);
    if (upDot > MAX_BOUNCE_UP) {
      s.forward.addScaledVector(s.groundNormal, MAX_BOUNCE_UP - upDot).normalize();
    }
  ...
  return {
    ...
    update(input, dt) {
      ...
      // The PLAYER MODEL (Stage 5) reads state.{position,forward,up,mode,
      // bank,walkSpeed,wingExtension,…} to render + animate the avatar; the
      // physics core keeps no mesh. Model sync runs after the world's
      // floating-origin rebase, from state.position.
    },
    setReadoutHook(hook) {
      readoutHook = hook;
    },
    beginFlight(position, forward, speed) {
      // The dominant body under the launch point (its surface normal, spin and
      // orbital velocity seed the flight). Fall back to the spawn home body.
      const dom = world.gravityAt(position, state.gravity).dominant ?? home;
      if (!dom) return;
      state.position.copy(position);
      const up = dom.upAt(position, _launchUp);
      state.up.copy(up);
      state.groundNormal.copy(up);
      // Heading: the supplied forward flattened onto the local tangent plane.
      // Degenerate (forward ∥ up) → a stable seed, as at spawn.
      state.forward.copy(projectTangent(forward, up));
      if (state.forward.lengthSq() < 1e-8) {
        _launchSeed.set(Math.abs(up.x) < 0.9 ? 1 : 0, 0, Math.abs(up.x) < 0.9 ? 0 : 1);
        state.forward.copy(projectTangent(_launchSeed, up));
      }
      state.forward.normalize();
      state.bodyRight.crossVectors(up, state.forward).normalize();
      state.walkSpeed = 0;
      enterFlyingFromGround(dom, speed ?? PLAYER.jumpTakeoffSpeed, 1.5);
    },
  };
