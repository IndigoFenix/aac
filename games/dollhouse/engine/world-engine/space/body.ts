...
/** A scatter obstacle (tree/rock) on a body's surface — the collision shape
 *  the flight physics reads. Full render data lives in the scatter module
 *  (Stage 4c). */
...
/** Per-body cloud renderer (Stage 4c refines this). */
...
export interface CelestialBody {
  ...
  /** The body's baked geography — the shared world model that generated its
   *  terrain (grid, substrate fields, FOUNDING SITES). Set by `materialize`
   *  for walkable bodies; the civ layer reads `geography.sites` to put
   *  cities on the world. Undefined until the geology bakes (and always for
   *  stars / gas giants). */
  geography?: BuiltPlanet;
  /** Async-bake surface (walkable bodies created with a `bakeGeography`
   *  service). START gate: kick the off-thread bake (idempotent — the world
   *  calls this well before the mesh pixel gate). */
  startGeographyBake?: () => void;
  /** True while a walkable body still awaits its baked terrain — the host's
   *  FORCE gate reads this to decide whether the player outran the bake. */
  geographyPending?: boolean;
  /** Resolves when the running bake installs its terrain. */
  whenGeography?: Promise<void>;
