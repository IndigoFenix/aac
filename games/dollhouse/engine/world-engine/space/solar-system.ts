...
import { createCelestialBody, type GeographyBaker } from "./celestial-body.js";
...
export interface GenerateSystemOpts {
  ...
  faceN?: number;
  /** Off-thread geography bake service (see CreateBodyOpts.bakeGeography). */
  bakeGeography?: GeographyBaker;
  /** BODY-SIZE COMPRESSION (space-time-compression.md §5): a miniature system.
   *  Body radii ÷ s and masses ÷ s² (surface gravity preserved — see
   *  CreateBodyOpts.compression). Default 1 (real scale). */
  compression?: number;
  /** INTERPLANETARY DISTANCE compression (settlement-emergence.md §4a): orbit
   *  semi-major axes ÷ this. **Defaults to `compression`** — the whole system
   *  shrinks uniformly and relative scales are preserved, which is the law.
   *  Raising it above `compression` pulls orbits in without shrinking bodies,
   *  so neighbours LOOM in the sky (scale.ts `apparentSizeGain`). Periods
   *  follow Kepler over the scaled axis, then `revolution` divides them. */
  interplanetary?: number;
  /** ORBITAL-PERIOD multiplier — every orbit in the system completes this many
   *  times faster than Kepler alone would have it. This is the world's
   *  `revolution` dial reaching the sky: a declared year length is a real
   *  planet going round faster, not a number typed in seconds. Default 1. */
  revolution?: number;
  /** SPIN multiplier — every body turns this many times faster (the world's
   *  `rotation` dial). Default 1. */
  rotation?: number;
}
...
export function generateSolarSystem(opts: GenerateSystemOpts): SolarSystem {
  ...
  // The distance dial defaults to the BODY dial: uniform shrink, relative
  // scales preserved (the universe-wide compression law).
  const interplanetary = Math.max(1, opts.interplanetary ?? compression);
  const revolution = Math.max(1e-6, opts.revolution ?? 1);
  const rotation = Math.max(1e-6, opts.rotation ?? 1);
  ...
  for (const rb of ordered) {
    ...
    if (parentCB) {
      const aMeters = (pb.formationSemiMajor * AU_M) / interplanetary;
      initialPosition = parentCB.worldPosition.clone().add(new THREE.Vector3(aMeters, 0, 0));
      // Kepler over the scaled axis, then the world's declared orbital
      // acceleration. The orbit is KINEMATIC (an explicit period), so a
      // declared year is simply this body going round that much faster —
      // compression is presentation, and this is where it lands.
      const keplerPeriod = 2 * Math.PI * Math.sqrt(aMeters ** 3 / Math.max(1, parentCB.gm));
      const period = keplerPeriod / revolution;
    ...
    const { body } = createCelestialBody({
      resolved: rb, systemSeed: star.systemSeed, initialPosition, orbit,
      faceN: opts.faceN, bakeGeography: opts.bakeGeography, compression, rotation,
    });
