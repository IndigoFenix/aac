...
export interface NpcErrandPoint extends Vec2 {
  dwell?: number;
  arrive?: number;
  /** FOLLOW THE PLAN EXACTLY to this point — never bend the aim around a local
   *  obstacle (`detourAim`). Set by the router on every point it placed INSIDE a
   *  building: an indoor route is planned through gaps measured in centimetres,
   *  so a local bypass can only drag the body off the verified corridor and into
   *  the very furniture the route was built to skirt. Outdoors the bend still
   *  earns its keep (open ground, unmodelled obstacles). */
  tight?: boolean;
...
/**
 * IS THIS WAYPOINT ONE THE BODY MUST WALK EXACTLY? — the single reading of the
 * no-corner-cutting law, shared by every consumer:
 *   • `errandLegTight()`, which the host uses to veto the aim bend (detourAim)
 *     AND the engine's wall glide;
 *   • the pure-pursuit carrot, which may not chord ACROSS such a point;
 *   • the skip-ahead recovery, which may not drop such a point.
 * `tight` is the router's explicit say-so (every point it placed inside a
 * building). The `arrive` heuristic predates it and stays for legs nobody
 * tagged — a small arrival radius has always meant "threading something"
 * (a door transit's 0.4, an indoor dogleg corner's 0.5).
 */
function pointTight(pt: NpcErrandPoint): boolean {
  return pt.tight === true || (pt.arrive ?? 1) < 0.8;
}
...
class BaseController implements NpcController {
  ...
  /**
   * May the host bend this leg's aim around a local obstacle, or must the body
   * hold the planned line? TIGHT = hold the line.
   *
   * `tight` is the router's explicit say-so (every point it placed indoors).
   * The `arrive` heuristic below predates it and stays for legs nobody tagged —
   * a small arrival radius has always meant "threading something".
   *
   * ⚠️ THE FINAL WAYPOINT IS WHY THIS FLAG EXISTS. `doorRouteErrand` gives its
   * intermediate points an `arrive` but deliberately leaves the ENDPOINT
   * without one (the caller's exact spot + dwell must survive), so the endpoint
   * read as `arrive = 1` ⇒ NOT tight — and the local detour switched on for the
   * one leg that actually threads the furniture. Observed: a body sent to the
   * clothes chest planned a correct 3-point route round its own dining table,
   * committed to the final leg from the wrong side, and the bend then aimed it
   * across the table; it pinned against the table's west face at the collider
   * boundary (x frozen to the millimetre while y jittered) and re-routed 50+
   * times without ever moving.
   */
  errandLegTight(): boolean {
    const pt = this.errand?.points[this.errandIndex];
    return !!pt && pointTight(pt);
  }
   ...
   *
   *  TIGHT vertices (pointTight — every point the router placed indoors, and
   *  every door transit) are passed by projection like any flow vertex, but the
   *  carrot may not walk its budget THROUGH one: it clamps there (reach-extended,
   *  so pace is kept) and the aim points down the planned line. Without that
   *  clamp the lookahead chords across routed corners spaced a few centimetres
   *  apart and the body clips the fixture the route went around — the corner-cut
   *  the `tight` flag exists to forbid. Outdoors nothing is tight and the carrot
   *  flows through vertices exactly as before.
   *
  ...
  private errandAim(ctx: NpcControlCtx): Vec2 | null {
    ...
    //    (never scanning past a STOP vertex — a dwell must not be skipped, nor
    //    past a TIGHT one — a routed indoor corner is not a vertex the body may
    //    drop: skipping it IS the corner-cut, and "the body already stands
    //    beside the next leg" is precisely what a fixture-grazing stray looks
    //    like) and resume from there, passing the skipped flow vertices properly.
    {
      const SKIP_NEAR = 0.8;
    ...
    for (;;) {
      ...
      // half a metre out hands the next leg an aim the body takes on the DIAGONAL,
      // and that diagonal is the inside of the corner — the fixture. It costs
      // nothing in reachability, because the carrot below is clamped ON a tight
      // vertex: the body is already driving straight at it (reach-extended, so it
    ...
    for (;;) {
      ...
      // TIGHT vertex — clamp the carrot ON it instead of walking the budget
      // THROUGH it. The lookahead (1.1 m) dwarfs the indoor router's corner
      // spacing (its cells are ≤ 0.25 m), so a carrot allowed past the vertex
      // sits two or three corners downstream and the body chords the whole
      // dogleg — cutting exactly the fixture corner the route was measured to
      // round. Observed: the body passed 0.13 m CLOSER to a table than any
      // routed point did, on a route whose own margin was 0.13 m.
      //   Reach-EXTENDED (unlike a stop): the aim is projected out to AIM_REACH
      // so the body keeps walking pace at the corner. Aiming at the bare vertex
      // would brake into it — the historical "brake-stalled tight arrival".
      // Direction tracks the plan; reach sets the pace, as everywhere else here.
      if (isTight(i) && remaining <= budget) return reach(pt);
      if (remaining >= budget) {
  ...
  private wanderAim(ctx: NpcControlCtx): Vec2 | null {
    ...
    if (!this.waypoint) {
      const p = this.pickWaypoint(ctx);
      this.stuckFrom = null;
      if (!p) {
        // NOWHERE REACHABLE to roam (a body shut inside a building it
        // doesn't know its way out of): STAND rather than blind-aim.
        // Wander has no router — aiming at unreachable ground only pins
        // the body against a wall until stuck-detection fires, forever.
        // The host is the one with a router; it walks strays out.
        this.pauseUntil = ctx.now + 2 + ctx.rng() * 2;
        return null;
      }
      this.waypoint = p;
    }
  ...
  private pickWaypoint(ctx: NpcControlCtx): Vec2 | null {
    ...
    // Home only if the body can actually WALK there in a straight line. The
    // unconditional `home` fallback was the stuck-indoors bug: a body left
    // inside a building (a recruited builder whose walls rose around it, a
    // stray visitor) drew every candidate near its far-away home, failed
    // them all, then blind-aimed at home THROUGH the wall — and ground
    // there, repicking the same doomed aim forever. Unreachable ⇒ null:
    // the body stands, honestly, until something with a router moves it.
    const home = { ...this.home };
    return this.corridorClear(ctx, home, radius) && ctx.walkable!(home, radius) ? home : null;
  }
