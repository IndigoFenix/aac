// shared/world-engine/interaction/behavior/creatures.ts
//
...
export interface NeedTarget {
  ...
  state?: string;
  /**
   * OR an AFFORDANCE the item must carry ("play", "read") — resolved against
   * the concept library, so the need selects by what a thing CAN DO rather
   * than by what it is. This is what keeps a motive off a named fixture: the
   * fun need wants "something that affords play", not "the toybox".
   */
  affords?: string;
}
...
export interface CreatureState {
  id: CreatureId;
  /** LIKES — item instance ids and/or KIND words ("apple"): a liked kind values
   *  EVERY instance of the kind at baseline (receiving one creates debt) and
   *  VOICES a matching category want (dialogue) — preference, never a filter. */
  likes: string[];
  needs: CreatureNeed[];
  ...
  condition?: string;
  /**
   * The GENERIC fact store (facts.ts — creature-knowledge.md): itemState/
   * condition/presence beliefs keyed by factKey, written by perceiveFact =
   * tellFact (sight and dialogue are one channel). ADDITIVE: the certified
   * `knowledge`/`knownWants` stores above remain the location/want arms'
   * single writers until the deliberate step-3 unification. Absent = empty.
   */
  facts?: Record<string, import("./facts.js").Fact>;
  /**
   * PUZZLE-SAFETY FLAG (npc-behavior-and-town-economy.md §8): keep this creature at
   * its post. When set, the creature does NOT pursue its OWN needs by leaving —
   * `selectGoal` contributes no need-derived goals for it, so it stays put and waits
   * (a vendor at its counter, a quest-giver who mustn't wander off to solve its own
   * ask). The design law is ONE behavior model for all creatures; a puzzle creature
   * is a regular one wearing this thin flag, never a separate code path. It still
   * converses, answers, and obeys rules — it just won't self-provision.
   */
  cannotLeavePost?: boolean;
  /**
   * CAPABILITY: can this body work lids, latches and purses? Species data —
   * absent = true (the person default). Without grasp, need-ctx resolution
   * offers NO openable containers and NO market buys, so an unreachable want
   * simply BLOCKS and surfaces — which is what lets a housemate ADOPT it (the
   * pet gets fed, the sick get served) instead of a hard-coded petcare chore.
   * Open surfaces (a table, a floor bowl) stay reachable to everyone.
   */
  grasp?: boolean;
}

/** Can this creature work lids/latches/purses? (absent = yes — see `grasp`). */
export const canGrasp = (c: Pick<CreatureState, "grasp"> | undefined): boolean => c?.grasp !== false;
...
/** Where an item is, as far as a creature knows. The SAME monotone channel also
 *  carries PROVISION facts (semantic-behavior.md §2b): keyed `provides:<type>`
 *  (providesKey), a `provided` entry names the PLACE-FACT subject where that
 *  resource TYPE can be got ("buy:good:food") — one fact channel, so telling and
 *  asking spread provider knowledge exactly like sightings. */
export type ItemLocation =
  | { kind: "held"; by: CreatureId }
  | { kind: "loose" }
  | { kind: "provided"; place: string };
...
export type CreatureEvent =
  ...
  /** A generic FACT landed (facts.ts perceiveFact/tellFact) — the itemState/
   *  condition/presence twin of `knowledge-gained`. Type-only import: no
   *  runtime cycle (facts.ts imports seeItem from here). */
  | { type: "fact-learned"; creatureId: CreatureId; fact: import("./facts.js").Fact };
...
export interface CreatureSeed {
  id: CreatureId;
  /** Instance ids and/or kind words ("apple") — see CreatureState.likes. */
  likes?: string[];
  needs?: {
    itemId: ItemId;
    value: number;
  ...
  condition?: string;
  /** Puzzle-safety flag: stay at post, don't self-provision (see CreatureState). */
  cannotLeavePost?: boolean;
  /** Capability: can work lids/latches/purses (see CreatureState; absent = true). */
  grasp?: boolean;
}
...
export function createCreatureWorld(creatures: CreatureSeed[], items: ItemSeed[]): CreatureWorld {
  ...
  for (const seed of creatures) {
    world.creatures[seed.id] = {
      ...
      ...(seed.cannotLeavePost ? { cannotLeavePost: true } : {}),
      ...(seed.grasp === false ? { grasp: false } : {}),
    };
  }
...
export function valueTo(creature: CreatureState, item: ItemState): number {
  ...
  return likesItem(creature, item) ? BASELINE_LIKE_VALUE : 0;
}
 ...
 *  matches every apple)? */
export function likesItem(
  creature: Pick<CreatureState, "likes">,
  item: Pick<ItemState, "id" | "kind">,
): boolean {
  return creature.likes.includes(item.id) || (!!item.kind && creature.likes.includes(item.kind));
}

/** Seed/replace a creature's likes (the host gives each resident 1–2 fruit kinds). */
export function setLikes(world: CreatureWorld, creatureId: CreatureId, likes: readonly string[]): void {
  const creature = world.creatures[creatureId];
  if (creature) creature.likes = [...likes];
}

/** The first LIKED glyph among `available` (HEAD-matched: "apple.big" answers an
 *  "apple" like), in likes order — the host's take-choice at a stall. Returns the
 *  AVAILABLE glyph, or undefined when nothing on offer is liked. */
export function preferredOf(
  likes: readonly string[],
  available: readonly string[],
): string | undefined {
  for (const like of likes) {
    const hit = available.find((g) => headOf(g) === like);
...
export function seeItem(
  ...
  const same =
    prior &&
    prior.kind === where.kind &&
    (prior.kind !== "held" || (prior as { by: string }).by === (where as { by: string }).by) &&
    (prior.kind !== "provided" || (prior as { place: string }).place === (where as { place: string }).place);
  if (same) {
    return []; // nothing new
  }
...
/** The knowledge KEY a resource TYPE's provider is filed under ("provides:food").
 *  `type` is the sort word a want names — a category ("food") or a kind ("apple"). */
export const providesKey = (type: string): string => `provides:${type}`;
...
export function learnProvides(
  ...
  return seeItem(world, creatureId, providesKey(type), { kind: "provided", place });
}

/** The first provider this creature knows for any of the given type words (a
 *  want's kind, then category), or undefined. Feeds the generosity gate's
 *  `knowsSource` and the where-is redirect — a no becomes "get it at the market". */
export function knownProvider(
  creature: CreatureState,
  types: (string | undefined)[],
): string | undefined {
  for (const t of types) {
    const fact = t ? creature.knowledge[providesKey(t)] : undefined;
    if (fact?.kind === "provided") return fact.place;
  }
  return undefined;
}
...
/**
 * The owner CONSENTS to hand `itemId` to `requesterId` — the naked grant half of
 * `requestItem`'s accept (pending until physically TAKEN; concludeTransfer moves
 * ownership and clears whatever debt exists). The generosity gate (willingness.ts,
 * semantic-behavior.md §4b) calls this when a RELATIONAL give (affinity / surplus)
 * clears WITHOUT a covering debt; `requestItem` itself stays the debt-only rule.
 */
export function grantItem(
  ...
  const owner = world.creatures[ownerId];
  const item = world.items[itemId];
  if (!owner || !item || item.ownerId !== ownerId || item.bound) return [];
  owner.knownWants[itemId] = requesterId;
  item.pendingTransferTo = requesterId;
  return [{ type: "transfer-pending", itemId, from: ownerId, to: requesterId }];
}
