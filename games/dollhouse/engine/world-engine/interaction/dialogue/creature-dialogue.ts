...
import { ACTIVITY_REFUSAL } from "@shared/world-engine/interaction/dialogue/intent-lines.js";
...
export type DialogueActKind =
  ...
  | "invite" // "eat with me" / "play with me" — ASK to do an activity TOGETHER
...
export interface DialogueAct {
   ...
   *  invite: the ACTIVITY being asked for ("eat", "play"). */
  verb?: string;
...
export interface ProjectionOpts {
   ...
   *  "sing with me" on the board with no edit to this module. Empty/absent ⇒
   *  no invitations are offered. */
  jointActivities?: readonly string[];
  /** CAN this creature join that activity right now — is there a place, a free
   *  station, room on the roster, a gathering not already underway? The
   *  feasibility half of an invitation's answer (rituals.ts `ritualAnswer`);
   *  the WILLING half is the sociability gate here. Absent ⇒ no invitations. */
  canJoin?: (creatureId: CreatureId, verb: string) => boolean;
...
/** "[you +] {activity} + with + i_me" — the invitation, over the same `with`
 *  preposition the stay-with company ask already speaks. Level a is the
 *  ACTIVITY (the teaching point); level b adds the company; c is the sentence. */
const inviteAsk = (verb: string): LeveledGlyphs =>
  phrase({ subject: "you", verb, key: verb, tail: { join: "with", symbol: "i_me" } });
...
export function projectDialogue(
  ...
  // DO IT WITH ME — an invitation to a shared activity, offered for each thing
  // this culture actually gathers for that this creature COULD join right now.
  // Available in both dialogue states: asking someone to dinner has nothing to
  // do with whether they want something from you.
  for (const verb of opts.jointActivities ?? []) {
    if (!opts.canJoin?.(creatureId, verb)) continue;
    acts.push({ kind: "invite", verb, glyph: at(inviteAsk(verb), level) });
  }
...
export function selectAct(
  ...
  switch (act.kind) {
    ...
    case "invite": {
      ...
      // say WHICH: CAN it (a place, a seat, a gathering not already underway —
      // the world's `canJoin`, backed by rituals.ts) and WILL it (sociability
      // toward this particular asker). A creature that WANTS the activity
      // accepts anybody; a cold one, or one that dislikes the asker, does not.
      //
      // ⚠️ This produces NO events. Accepting an invitation does not move a
      // body — the HOST marks the invitation and the ordinary ritual loop does
