...
function BadgeStack(props: BadgeStackProps): React.ReactElement | null {
  ...
  for (const modKey of slot.modifiers) {
    ...
    if (item?.modifier && BADGE_TRANSFORMS.includes(item.modifier.transform)) {
    ...
    } else if (!item && !isAppearanceOnlyFacet(modKey)) {
      ...
      //
      // APPEARANCE-ONLY facets are excluded: `material_wood` says how a thing
      // LOOKS in the 3D world (variations.ts) and has no word and no art, so
      // routing it here drew a broken badge on the board. Same rule the
      // language layer applies when it skips it in the adjective walk — one
      // definition, in the module that owns the dimension.
