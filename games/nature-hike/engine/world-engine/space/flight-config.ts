...
export const PLAYER = {
  ...
  // Landing generosity: thresholds must sit ABOVE the speeds the governor
  // actually delivers a ship at (MIN_FLIGHT_SPEED 32 was > the old bounce
  // threshold 25 — a minimum-speed touch could never land cleanly).
  landBounceSpeed: 45,        // ~160 kph bounce threshold
  landStunSpeed: 90,          // ~324 kph stun threshold
...
export const SPEED = {
  ALTITUDE_ACCEL_FACTOR: 8.3, // (unused by the wheel-only target; kept for tuning history)
  /** Base cruise at wheelFactor 1 (m/s). The WHEEL IS THE SPEED: target =
   *  MIN_FLIGHT_SPEED × wheelFactor, constant at any altitude — the old
   *  altitude governor auto-braked/boosted against the pilot's intent. */
  MIN_FLIGHT_SPEED: 32,
  RHO_REF_AIR: 1.225,
