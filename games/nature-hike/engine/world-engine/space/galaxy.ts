...
/** DEBUG-ONLY provenance sink for the projector (the single-frame star-flash
 *  hunt). When a probe is passed, every emitted point records WHERE it came
 *  from — tier, source cell, distance, brightness, sprite size — index-aligned
 *  with the position/colour/size buffers, so a flagged frame can name the exact
 *  cell that produced a screen-filling blob.
 *
 *  Null in every production call: the projector runs per frame over ~10⁴ points
 *  and the arrays would allocate. `projectGalaxy` only touches it when non-null. */
export interface ProjectionProbe {
  /** LOD tier that emitted the point (0-3; MAX_LEVEL = individual stars). */
  tier: number[];
  /** "aggregate" (a smudged cell blob) or "star" (an expanded leaf star). */
  kind: string[];
  /** Source cell key / star id. */
  id: string[];
  /** Distance from the player to the source, ly. */
  distLy: number[];
  /** Fade weight applied (innerWeight × outerWeight). */
  weight: number[];
  /** Final per-channel brightness multiplier written into outColors. */
  k: number[];
  /** gl_PointSize written into outSizes. */
  pix: number[];
}

export function makeProjectionProbe(): ProjectionProbe {
  return { tier: [], kind: [], id: [], distLy: [], weight: [], k: [], pix: [] };
}

function resetProbe(p: ProjectionProbe): void {
  p.tier.length = 0; p.kind.length = 0; p.id.length = 0;
  p.distLy.length = 0; p.weight.length = 0; p.k.length = 0; p.pix.length = 0;
}
...
export function projectGalaxy(
  ...
  probe: ProjectionProbe | null = null,
): number {
  if (probe) resetProbe(probe);
  let count = 0;
    ...
    if (tier === 0) {
      for (const cell of universe.topCells) {
        count = projectCell(
          ...
          outPositions, outColors, outSizes, count, probe,
        );
      }
    } else {
...
function projectCell(
  ...
  if (cell.level === MAX_LEVEL) {
    ...
    for (const s of stars) {
      ...
      outSizes[count] = 3.0;
      if (probe) {
        probe.tier.push(cell.level); probe.kind.push("star"); probe.id.push(s.id);
        probe.distLy.push(distLy); probe.weight.push(weight);
        probe.k.push(k); probe.pix.push(3.0);
      }
      count++;
    }
    return count;
  ...
  outSizes[count] = pix;
  if (probe) {
    probe.tier.push(cell.level); probe.kind.push("aggregate");
    probe.id.push(cellKey(cell.level, cell.ix, cell.iy, cell.iz));
    probe.distLy.push(cellDist); probe.weight.push(weight);
    probe.k.push(k); probe.pix.push(pix);
  }
  return count + 1;
}
