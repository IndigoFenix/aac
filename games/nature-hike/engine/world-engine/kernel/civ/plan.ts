...
export function planHistory(prep: TriPrep, opts: PlanOpts): PlannedHistory {
  ...
  const charter = (cell: number): TriCharter => ({
    ...
    timberland: grid.fields.eco_tree
      ? boxSum(grid.fields.eco_tree, cell, charterR) * ECO_VEG_SCALE
      : boxSum(grid.fields.plant, cell, charterR),
    pasture: grid.fields.eco_grass
      ? boxSum(grid.fields.eco_grass, cell, charterR) * ECO_VEG_SCALE
      : 0,
  });
