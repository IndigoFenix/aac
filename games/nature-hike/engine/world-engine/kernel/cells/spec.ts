...
export interface VarSpec {
   ...
   *
   *  `outletBelow` is the SEA LINE in potential units: where water leaves the
   *  world. It's what lets the engine fill quantization pits (grid.fillDepressions)
   *  — without an outlet there is no "spill level" to fill toward, and the
   *  drainage shatters into terminal puddles. Omit on worlds with no sea.
   *
   *  `outletEdge` says this grid is a WINDOW on a bigger world (a region chart),
 ...
 *
 *  MULTI-REAGENT form (world-content.md §3d): `inputs` lists several
 *  reagents and the output is the Leontief min across ALL of them —
 *  output := min(min_i(input_i × eff_i), capacity). Tools need metal AND
 *  planks; the scarcest reagent binds. Exactly one of `input`/`inputs`. */
export interface ProcessSpec {
  id?: string;
  input?: string;
  output: string;
  /** Output per unit input (single-input form). */
  efficiency?: number;
  /** Multi-reagent Leontief: output ≤ input_i × efficiency_i for EVERY i. */
...
/** FAN-IN — the additive combinator (world-content.md §3d): a commodity
 *  with several demand sources (households burn timber AND the shipyard
 *  consumes it) needs a SUM, and process outputs overwrite ("later wins").
 *  output := Σ scale_i × term_i, clamped to the var's bounds, written each
 *  step as a DERIVED value — a pure combinational relay, same termination
 *  and loop-dimension story as `satisfied`. Point a flow net's `demand`
 *  at it to aggregate a settlement's total want. */
export interface SumSpec {
  id?: string;
  output: string;
  terms: Array<{ scalar: string; scale?: number }>;
}

/** FAN-OUT — the conserving per-settlement allocator (world-content.md
 *  §3d): processes are pure READS, not draws, so two processes consuming
 *  the same delivered input would each transform the full amount. The
 *  allocator splits `source` across consumers IN ARRAY ORDER (priority is
 *  boring data — the planned-history provider must reproduce it
 *  bit-for-bit): share_i := min(demand_i, remaining); remaining −=
 *  share_i. Conserving by construction (Σ shares ≤ source), derived, no
 *  dynamics of its own. Consumers read their share as a process input. */
export interface AllocateSpec {
  id?: string;
  /** The delivered quantity being divided (typically a net's satisfied). */
  source: string;
  /** Consumers in PRIORITY ORDER: first listed, first fed. */
  shares: Array<{ output: string; demand: string }>;
}
...
export interface WorldSpec {
  ...
  /** Fan-in relays (§3d): run after processes, before flow nets — a sum
   *  of this step's draws can be this step's net demand. */
  sums?: SumSpec[];
  /** Fan-out relays (§3d): run after flow nets — they divide this step's
   *  deliveries; consumers see their share NEXT step (one settle-step
   *  per chain stage, as ever). */
