...
const GAME_NAME = "nature-hike";

// This game builds against its VENDORED engine snapshot (games/nature-hike/engine),
// not live shared/world-engine — refresh it explicitly with
// `npm run sync:game-engine -- nature-hike`. The snapshot also carries the
...
if (!existsSync(ENGINE)) {
  throw new Error(
    "games/nature-hike/engine missing — run `npm run sync:game-engine -- nature-hike` first.",
  );
}
