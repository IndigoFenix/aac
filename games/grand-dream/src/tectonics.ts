...
// --- tuning ------------------------------------------------------------
const OCEAN_T = 7; // fresh ocean-floor thickness
const CONT_T = 30; // continental cap thickness
const MAX_THICK = 70;
const ELEV_BASE = 24; // thickness at sea level
const ELEV_SCALE = 1.6; // height units per thickness unit
export const SEA_HEIGHT = 3; // baked height below this = submarine
const SUBDUCT_UPLIFT = 0.3; // winner keeps this × the subducted slab
const OROGENY_KEEP = 0.75; // winner keeps this × consumed continental crust
...
const OROGENY_DAMP = 0.995;
const TALUS = 8; // elevation diff below which slopes hold (mountains keep steep flanks)
const ERODE_RATE = 0.025; // × excess slope, per epoch
const HOTSPOT_BUILD = 0.3; // volcano growth per epoch over a plume
// Ore emplacement lotteries: probability per event, richness, burial depth.
const ORE_ARC = { p: 0.18, amt: 6, cover: 3 };
const ORE_SUTURE = { p: 0.5, amt: 10, cover: 8 };
const ORE_RIFT = { p: 0.03, amt: 3, cover: 0 }; // surface evaporites
const ORE_HOTSPOT = { p: 0.04, amt: 8, cover: 5 };
