...
export async function generateBERTAACSymbols(context: AACContext, language: string = "en", usePCS: boolean = false, userId?: string): Promise<PrAACTSymbol[]> {
  try {
    ...
    const useChatGPT5 = userId ? await shouldUseChatGPT5(userId) : false;
    let rawResponse: string | undefined;
    
    if (useChatGPT5) {
      console.log(`🚀 Using ChatGPT-5 override for BERT AAC symbols - User ${userId}`);
      try {
        ...
        const jsonResponse = await modelOverrideService.generateChatGPT5JSON(
          aacPrompt,
          systemInstruction,
          0.7
        );
