...
export async function analyzePersonAndRole(base64Image: string, userId: string): Promise<PersonAnalysis> {
  try {
    // Get user profile to understand who the main user is
    ...
    const userGender = user.gender || "unknown gender";
...
- Gender: ${userGender}
- Known people: ${knownPeople.length > 0 ? knownPeople.map(p => `${p.name} (${p.role})`).join(', ') : 'None registered'}
