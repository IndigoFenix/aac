...
6. **🟡 §17 / DR — no tested restore.** There is no backup/restore script, no `aws_backup_plan`, and no dated recovery drill; RDS `backup_retention_period` is the only implemented control.
...
| 5.7 Remote access: approval, role-scoped, logged | 🟡 | Bastion has **no ingress rules at all**, no public IP and no SSH key pair — access is AWS SSM Session Manager only (`terraform/bastion.tf`). Admin PHI reads are audited (`sharing/audit.ts`, `viaAdmin`) and support-mode impersonation is audited start/end with a 60-minute lapse. Not yet: SSM session logging, and per-engineer IAM database authentication (human DB sessions share `aivota_admin`). |
...
| 5.11.2 No routine Administrator/shared accounts | 🟡 | Application logins are personal and admin actions are audited. **Database** access is not: human sessions use the shared `aivota_admin` role over the SSM bastion. Per-engineer IAM database authentication is an open item. |
| 5.11.3 Remove unneeded services, patch | ✅ | Managed AWS services; `auto_minor_version_upgrade` on RDS. |
| 5.11.4 OS hardening + ongoing security updates | 🟡 | Fargate managed runtime; container runs non-root. Not done: `readonlyRootFilesystem` is unset, ECS Exec is enabled, and the image is deployed by the mutable `:latest` tag. The coturn EC2 host has no patch automation. |
