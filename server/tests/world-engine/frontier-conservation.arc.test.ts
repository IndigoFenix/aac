...
describe("frontier — item conservation across a construction haul arc", () => {
  ...
  it("baskets are CONSTANT once the pool has finished becoming visible", () => {
    // 🧺 THE PREMISE MOVED ON 2026-09-09 AND THE NET IS THE BETTER FOR IT. This
    // used to read "frontier has no weaver, no basket recipe and no basket
    // import, so over any arc at all this is a constant" — a net whose catch was
    // empty by construction. A basket IS makeable now (2 wood at the workbench)
    // and a body whose collection plan wants one CAN issue itself a bill, so the
    // constancy asserted here is a real claim about THIS arc: nothing in it
    // demands a vessel inside the window, and nothing mints one per trip. It
    // still holds (measured: flat over 400 sim-s at this point in the boot),
    // which is the positive half of the ③ finding — the transcripts' climb
    // 11→14→16 PLATEAUS at 16 and stays there for another 2 000 sim-s. If a
    // producer ever fires on this path, it lands here, which is what a net is
    // for.
    const before = units(run, "basket");
    run.advanceS(120);
    expect(units(run, "basket")).toBe(before);
