...
describe("world-engine net sync (two peers)", () => {
  it("propagates possession + position and hands a ball off cleanly", () => {
    ...
    framesUntil(
      ...
      () => ({ x: 52, y: 30 }),
      () => null,
      () => A.objects.ball.possessedBy === "A",
    );
    expect(A.objects.ball.possessedBy).toBe("A");

    // B learns about it over the wire, and sees the ball where A holds it.
    frames(A, B, { x: 52, y: 30 }, null, 0.2);
    expect(B.objects.ball.possessedBy).toBe("A");
    expect(Math.hypot(B.objects.ball.x - A.objects.ball.x, B.objects.ball.y - A.objects.ball.y))
      .toBeLessThan(0.5);

    // --- A kicks (hard brake) and the ball rolls free to rest. ---
    frame(A, B, null, null); // the braking frame → release
    expect(A.objects.ball.possessedBy).toBeNull();
    expect(B.objects.ball.possessedBy).toBeNull();

    frames(A, B, null, null, 4); // both coast; ball comes to rest
    expect(Math.hypot(A.objects.ball.vx, A.objects.ball.vy)).toBe(0);
    expect(A.objects.ball.freeRollOwner).toBeNull();
    expect(B.objects.ball.freeRollOwner).toBeNull();
    // The free-roll stayed in sync across the wire.
    expect(Math.abs(A.objects.ball.x - B.objects.ball.x)).toBeLessThan(0.6);
    ...
    const throughBall: Vec2 = { x: A.objects.ball.x - 8, y: A.objects.ball.y };
    framesUntil(
      A, B,
      () => null,
      () => throughBall,
      () => B.objects.ball.possessedBy === "B",
    );
    expect(B.objects.ball.possessedBy).toBe("B");

    frames(A, B, null, throughBall, 0.1);
    expect(A.objects.ball.possessedBy).toBe("B"); // the grab reached A over the wire
  });
