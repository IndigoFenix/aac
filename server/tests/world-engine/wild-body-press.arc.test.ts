...
describe("#50 ⑥ a founding settler takes the press", () => {
  ...
  it("🧑‍🌾 a press with an idle SETTLER in reach installs the errand on it", () => {
    const busy = (cid: string): boolean =>
      ...
      run.session.liveNeedBodies.has(cid) ||
      (run.session.npcTasks.get(cid)?.length ?? 0) > 0;
    // ⚖️ RE-FIXTURED 2026-09-10 (piles-not-boxes-round.md §17), SAME ASSERTION.
    //
    // This used to hunt for a settler that happened to be IDLE, and the hunt
    // stopped finding one: settlers now carry a provision row for the camp's
    // larder (`campLarderDraw`), so a founding group is busy nearly always. The
    // premise was never "some settler is idle" — MAIN'S RULING is that a body
    // serving its OWN errand is addressable, because an order pre-empts a need
    // (`idleForDirect`'s own-pursuit arm; the user's *"Player orders should take
    // high priority"*). So the fixture takes the FIRST settler, busy or not, and
    // the assertion below is untouched: the press must land on it and install a
    // real errand. `busy` is still read — at the END, to prove the command took.
    const cid = settlers()[0];
    if (!cid) throw new Error("no settler at all to address — fixture broken, not a finding");
