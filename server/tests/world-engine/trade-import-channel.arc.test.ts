...
describe("T3 — the cargo list is a READING, and the bind is only when it starts", () => {
  ...
  it("🚨 THE WORST SHORTAGE GETS THE LARGEST SHARE — and Σ is the allotment exactly", () => {
    ...
    const c = center();
    tr.bindPartner({ key: "hamlet", at: { x: c.x + 600, y: c.y }, distanceM: 600 });
    // 🚨 RE-FIXTURED, PREMISE MOVED (trade-topology round, ruling 3) — the
    // SAME assertions (Σ = 6, strictly ordered, a good off the line lands 0)
    // and the same expected split [3, 2, 1]; ONE input moved, derived below.
    //
    // The hold used to be dealt by the 0..1 `want`; it is now dealt by LANDED
    // ADVANTAGE in hand-seconds, which prices each good's freight against what
    // relieving the shortage is worth. `wood` is rawBulk (valueDensity 0.5),
    // so over this 600 m road it costs 600/(15.2×0.5) = 78.947 s a unit — and
    // at the old shortage of 0.2 it was only worth 0.2×240 = 48 s, i.e. the
    // caravan would have DESTROYED value carrying it, and the worthwhile gate
    // drops it off the lane entirely (asserted at the end). Its break-even
    // shortage on this road is 78.947/240 = 0.329, so the fixture's third good
    // now sits above it at 0.6:
    //   cloth     1.0×240 − 600/(15.2×4)   = 240 −  9.868 = 230.132
    //   clothing  0.5×240 − 600/(15.2×8)   = 120 −  4.934 = 115.066
    //   wood      0.6×240 − 600/(15.2×0.5) = 144 − 78.947 =  65.053
    const them = { shortage: () => 0 };
    const us = {
      shortage: (g: string) =>
        ({ cloth: 1, clothing: 0.5, wood: 0.6 } as Record<string, number>)[g] ?? 0,
    };
    ...
    // 6 units weighted 230.132 / 115.066 / 65.053 (Σ 410.251) = 3.366 / 1.683
    // / 0.951 → floors 3 / 1 / 0, and the two left over go to the largest
    // remainders (wood .951, clothing .683).
    expect(units).toEqual([3, 2, 1]);
    expect(units.reduce((a, b) => a + b, 0)).toBe(IMPORT_ALLOTMENT); // conserved
    // Strictly ordered by landed gain — the flat split gave all three 2 apiece.
    ...
    // …and so does one the ROAD prices off the line: put wood back under its
    // break-even and the same books, the same partner and the same road drop
    // it, leaving the hold to the two that repay the trip.
    tr.refreshCargo({
      us: { shortage: (g: string) => ({ cloth: 1, clothing: 0.5, wood: 0.2 } as Record<string, number>)[g] ?? 0 },
      them,
      goods: ["cloth", "clothing", "wood"],
    });
    expect([...tr.route.imports]).toEqual(["cloth", "clothing"]);
    expect(tr.importUnitsPerVisit("wood")).toBe(0);
    expect(tr.route.imports.reduce((n, k) => n + tr.importUnitsPerVisit(k), 0))
      .toBe(IMPORT_ALLOTMENT);
