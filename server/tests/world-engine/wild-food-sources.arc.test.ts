...
describe("⑥ a foraged plant bears again", () => {
  let run: TextQuestRun;
  /** The larder's armed count AT BOOT, read before the clock moves at all. */
  let armedAtBoot = -1;
  /** How many bearing plants the near stand actually holds at boot. */
  let larderAtBoot = 0;
  ...
  beforeAll(() => {
    ...
    // 🚨 THE BOOT WITNESS IS TAKEN AT BOOT, and it has to be (the
    // resource-packing round). This read used to sit inside ① after the
    // `advance(20)` below, which was sound only while the countryside was thin
    // enough that nobody could reach food in ten sim-seconds. The packed cell
    // stands ten bearing plants in the near disc (5 banana + 5 carrot, against
    // the two the authored densities left), so a settler now forages inside
    // the warm-up and the SECOND read would be a fact about foraging speed,
    // not about the boot.
    const species = new Set(foodPlants().map((r) => r.species));
    const boot = run.session.wilderness!.features.filter((f) => species.has(f.species));
    larderAtBoot = boot.length;
    armedAtBoot = boot.filter((f) => f.regrowAt !== undefined).length;
    run.advance(20);
  }, 600_000);
  ...
  it("① a settler's take ARMS the plant's regrow clock", () => {
    ...
    expect(larderAtBoot).toBeGreaterThan(0);   // the run is not vacuous
    expect(armedAtBoot).toBe(0);
