...
describe("⑨ F2 — the two currencies meet on ONE ladder", () => {
  ...
  it("LINK RANKING IS DERIVED — the cascade order equals the schema's rank", () => {
    // ⚖️ THE ORDER IS NO LONGER A LITERAL (emergent-plans-round.md D8). It used
    // to be `LINK_RANK = {build:0, haul:1, refine:2, fell:3}` written in the
    // reader; it is now `cascadeRank()` — `depthOf(CASCADE_SCHEMA, "built")`
    // folded by MIN onto the four link names. This pin reads the derivation
    // THROUGH `visibleBills`' output order, on a world with ALL FOUR links open
    // at once, so the schema and the reader cannot drift apart unnoticed.
    const f = makeFixture();
    f.stage();
    // The site is staged (BUILD) and short of two heads: `block`, which the
    // crate can serve (HAUL), and `wood`, whose only source is the wild forest
    // (FELL). A second order is a bench in labour (REFINE).
    (f.orders[0] as { costs?: Record<string, number> }).costs = { block: 130, wood: 5 };
    f.orders.push({
      kind: "refine",
      ord: 2,
      type: "house",
      ...
      buildDays: 2,
      costs: {},
    } as unknown as ConstructionOrder);
    f.at.set("resident_0_0", SITE_AT);
    const links = visibleBills(f.session, "resident_0_0", f.deps);
    const observed = links.map((l) => l.link);
    expect(observed).toEqual(["build", "haul", "refine", "fell"]);
    // …and that IS the schema's rank, read off the rows rather than asserted:
    const rank = cascadeRank();
    expect(observed.map((l) => rank[l])).toEqual([0, 1, 2, 4]);
    expect(observed).toEqual([...observed].sort((a, b) => rank[a] - rank[b]));
    // 🚨 4, not 3 — and it does not matter, which is the point. The rank is
    // only ever the comparator's first term, so the ORDER is the contract; the
    // ordinal pin lives in `cascade-schema.test.ts` beside the old literal.
  });
