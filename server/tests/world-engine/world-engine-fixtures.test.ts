...
describe("fixtures", () => {
  ...
  it("follows heldOpen (an explicit access action), NOT mere proximity", () => {
    const s = createWorldState(spec([chest]), "me");
    // Standing beside it no longer opens it — a creature must OPEN it to reach in.
    steerFor(s, 20, 16, 3); // walk up and stand at the chest
    expect(s.objects.chest.open).toBeLessThan(0.3);
    // Opened (the access action): the lid eases open and STAYS open.
    s.objects.chest.heldOpen = true;
    steerFor(s, 20, 16, 2);
    expect(s.objects.chest.open).toBeGreaterThan(0.6);
    steerFor(s, 10, 16, 2); // walk away — still open (until closed)
    expect(s.objects.chest.open).toBeGreaterThan(0.6);
    // Closed (done): it shuts.
    s.objects.chest.heldOpen = false;
    steerFor(s, 10, 16, 2);
    expect(s.objects.chest.open).toBeLessThan(0.3);
  });
