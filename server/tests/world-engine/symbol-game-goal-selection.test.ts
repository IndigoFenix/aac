...
import { makePersonality, personalityFromPreset } from "@shared/world-engine/interaction/behavior/personality.js";
...
describe("needToGoal — a creature pursues its own needs", () => {
  ...
  it("FETCH-FIRST: a hold-requiring need acquires the resource before delivering it", () => {
    // The shopping errand as ONE need: deliver food home. Not holding it → go get it
    // (the market trip); holding it → carry it home (putIn) — no script, just state.
    const deliver = { itemId: "food1", value: 3, placedAt: "pantry", target: { category: "food" } };
    expect(needToGoal(deliver, false)!.kind).toBe("fetch"); // haven't bought it yet
    expect(needToGoal(deliver, true)!.kind).toBe("putIn"); // got it → bring it home
    // Hand-off and transform needs are fetch-first too.
    const handoff = { itemId: "x", value: 2, forCreature: "cub", target: { category: "food" } };
    expect(needToGoal(handoff, false)!.kind).toBe("fetch");
    expect(needToGoal(handoff, true)!.kind).toBe("give");
    const cook = { itemId: "apple1", value: 2, requiresState: "hot" };
    expect(needToGoal(cook, false)!.kind).toBe("fetch");
    expect(needToGoal(cook, true)!.kind).toBe("transform");
  });
});

describe("selectGoal — the shopping errand EMERGES from need + inventory (§8.1b)", () => {
  const shopNeed = { itemId: "food1", value: 3, placedAt: "pantry", target: { category: "food" } };
  const runtimes = () => new Map<string, RuleRuntime>();
  const decide = (w: ReturnType<typeof createCreatureWorld>, id: string) =>
    selectGoal({
      ctx: { self: w.creatures[id]!, world: w, worldConditions: new Set() },
      rules: [],
  ...
  it("not holding the resource → the goal is to FETCH it (walk to the market)", () => {
    const w = createCreatureWorld([{ id: "shopper", needs: [shopNeed] }], []); // owns no food
    expect(decide(w, "shopper").chosen?.goal.kind).toBe("fetch");
  });

  it("holding the resource → the goal flips to carry it home (putIn), no market trip", () => {
    const w = createCreatureWorld(
      [{ id: "shopper", needs: [shopNeed] }],
      [{ id: "food1", ownerId: "shopper", kind: "bread", category: "food" }], // already bought
    );
    expect(decide(w, "shopper").chosen?.goal.kind).toBe("putIn");
  });
});

describe("cannotLeavePost — the thin puzzle-safety flag (§8)", () => {
  const runtimes = () => new Map<string, RuleRuntime>();

  it("a flagged creature does NOT pursue its own need (stays at post, idle)", () => {
    const w = createCreatureWorld(
      [{ id: "vendor", needs: [{ itemId: "apple1", value: 9 }], cannotLeavePost: true }],
      [{ id: "apple1", kind: "apple" }],
    );
    const res = selectGoal({
      ctx: { self: w.creatures.vendor!, world: w, worldConditions: new Set() },
      rules: [],
      relationTo: () => DEFAULT_RELATION,
      runtimes: runtimes(),
      now: 0,
    });
    expect(res.chosen).toBeNull(); // even a survival-level need is suppressed at post
  });

  it("but a flagged creature STILL obeys rules (the flag scopes only self-needs)", () => {
    const w = createCreatureWorld(
      [{ id: "bear", needs: [{ itemId: "apple1", value: 4 }], cannotLeavePost: true }, { id: "player" }],
      [{ id: "apple1", ownerId: "bear", kind: "apple" }],
    );
    const res = selectGoal({
      ctx: { self: w.creatures.bear!, world: w, worldConditions: new Set(["night"]) },
      rules: [goHomeRule()],
      relationTo: () => makeRelation({ authority: 1, trust: 1 }),
      runtimes: runtimes(),
      now: 0,
    });
    expect(res.chosen?.source).toMatchObject({ kind: "rule" });
  });
});
...
describe("compileGoal — the world adapter", () => {
  ...
  it("give → acquire the item first, then carry it to the recipient", () => {
    const plan = compileGoal(
      { kind: "give", item: { id: "apple1" }, to: "player" },
      "bear",
      resolver, // no carrierOf → the item reads as loose
    );
    expect(plan?.steps).toEqual([
      { kind: "moveTo", pos: { x: 2, y: 2 } }, // walk to the apple
      { kind: "pick", itemId: "apple1" },
      { kind: "moveTo", pos: { x: 10, y: 0 } }, // walk to the player
      { kind: "give", itemId: "apple1", to: "player" },
    ]);
  });

  it("give skips the pickup leg when the actor already holds the item", () => {
    const carrying: WorldResolver = { ...resolver, carrierOf: () => "bear" };
    const plan = compileGoal({ kind: "give", item: { id: "apple1" }, to: "player" }, "bear", carrying);
    expect(plan?.steps).toEqual([
  ...
  it("give refuses when someone ELSE holds the item (not snatchable)", () => {
    const held: WorldResolver = { ...resolver, carrierOf: () => "fox" };
    expect(compileGoal({ kind: "give", item: { id: "apple1" }, to: "player" }, "bear", held)).toBeNull();
  });

  it("putIn → acquire the item, then carry it to the container", () => {
    const plan = compileGoal(
    ...
    expect(plan?.steps).toEqual([
      { kind: "moveTo", pos: { x: 2, y: 2 } },
      { kind: "pick", itemId: "apple1" },
      { kind: "moveTo", pos: { x: 3, y: 3 } }, // the named place
