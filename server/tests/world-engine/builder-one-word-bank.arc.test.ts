...
//   (b) a FOUNDING session, NO house — `scripts/worlds/frontier-cell.spec.json`
//       (the GL `frontier-planet` premise's own founding cell, GENERATED from
//       the real planet: scope town, days 0, population 5, wilderness,
//       `terrain: "planet"` with the measured cell declared, no `initial_focus`)
//       ⚖️ planet-boot round: was `frontier-planet.spec.json`, which is now the
//       SOLAR document and bakes a planet — a 25 s cost this suite has no use
//       for, since what it pins is that the word bank does not vary with the
//       session KIND.
...
describe("ONE WORD BANK — the builder's vocabulary is session-blind", () => {
  ...
  beforeAll(() => {
    ...
    founding = bootAndCapture("frontier-cell.spec.json");
  });
  afterAll(() => {
    doll?.run.dispose();
