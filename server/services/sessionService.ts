...
const SYMBOL_LOG = join(new URL(".", import.meta.url).pathname.replace(/^\/([A-Z]:)/, "$1"), "..", "symbol-generation-debug.log");
function logSymbol(msg: string) {
  const line = `[${new Date().toISOString()}] [SessionService] ${msg}\n`;
  console.log(`[SymbolGen] ${msg}`);
  try { appendFileSync(SYMBOL_LOG, line); } catch (e) { console.error(`[SymbolGen] Log write failed:`, e); }
}

/**
 * Resolve imageKeys on a Context_Board: look up existing symbols in DB,
 * generate new ones via Gemini if the student has generateSymbols enabled.
...
async function resolveImageKeysOnBoard(board: any, studentId?: string): Promise<void> {
  ...
  for (const button of buttonsWithKeys) {
    try {
      const existing = await customSymbolRepository.getSymbolByKey(button.imageKey);
      if (existing) {
        button.symbolPath = `/api/custom-symbols/${existing.id}/image`;
      } else {
        toGenerate.push(button.imageKey);
      }
    } catch { /* ignore */ }
  }

  // Phase 2 (background): generate missing symbols — don't block the response
  if (generateEnabled && toGenerate.length > 0) {
    logSymbol(`Launching background generation for ${toGenerate.length} symbols: ${toGenerate.join(", ")}`);
    generateSymbolsInBackground(toGenerate).catch(err =>
      logSymbol(`Background generation error: ${err}`)
    );
  }
}

/**
 * Generate symbols sequentially in the background with rate limiting.
 * Symbols are saved to DB/S3 — clients poll by imageKey to pick them up.
 */
async function generateSymbolsInBackground(imageKeys: string[]): Promise<void> {
  let generated = 0;
  for (const key of imageKeys) {
    ...
    try {
      // Double-check it wasn't generated by another request in the meantime
      const existing = await customSymbolRepository.getSymbolByKey(key);
      if (existing) {
        logSymbol(`"${key}" already generated by another request — skipping`);
        continue;
      }

      logSymbol(`Generating symbol for "${key}"...`);
      const imageBuffer = await generateSymbolImage(key);
      ...
      generated++;
      logSymbol(`Generated "${key}" → ${symbol.id} (${generated}/${imageKeys.length})`);
    } catch (err: any) {
      const isQuota = err?.status === 429 || err?.message?.includes("quota") || err?.message?.includes("rate") || err?.message?.includes("RESOURCE_EXHAUSTED");
      if (isQuota) {
        logSymbol(`Quota exhausted after ${generated}/${imageKeys.length} — stopping`);
        break;
      }
      logSymbol(`ERROR generating "${key}": ${err}`);
    }
  }
  logSymbol(`Background generation done: ${generated}/${imageKeys.length}`);
}
