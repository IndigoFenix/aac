...
import { DEFAULT_SOCIAL_GAME, DOLLHOUSE_CALL_GAME, NATURE_HIKE_CALL_GAME } from "@shared/social-world/default-game";
...
export async function listSocialGameOptions(instituteId: string): Promise<CallGame[]> {
  // Built-ins first: the default world, then the Dollhouse and Nature Hike
  // iframe games (every participant mounts them locally; no spec travels with them).
  const options: CallGame[] = [DEFAULT_SOCIAL_GAME, DOLLHOUSE_CALL_GAME, NATURE_HIKE_CALL_GAME];
