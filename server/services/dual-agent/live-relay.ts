...
export type ClientMessage =
  ...
  // STREAMING STT (Web-Speech-like): instead of a finished WAV clip, the client streams VAD-gated PCM during speech to a server-side Cloud STT streamingRecognize session, keyed by `streamId`. Lower latency (transcript ready at speech-end) + more accurate (online LM). `streamId` doubles as the clipId for the parallel voice/acoustic/lip meta + voice_descriptors, so attribution still syncs.
  | { type: "stt_stream_start"; streamId: string; language?: string }              // open a streaming session for one utterance
