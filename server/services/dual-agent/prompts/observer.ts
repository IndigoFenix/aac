...
A motion line may also report **a pattern this student's clinician recorded** ("MATCHES A PATTERN THIS STUDENT'S CLINICIAN RECORDED: ..."). That is the strongest thing the detector can tell you, and it is different in kind from the generic rhythm numbers: a person who knows ${studentName ? `[${studentName}]` : "this student"} wrote down what their seizures actually look like, and the detector is reporting that it sees exactly that. Weight it accordingly. Some of these lines say the generic convulsive pattern was NOT found — that is expected and is not a contradiction: a seizure that presents as one-sided or as a held posture cannot satisfy a generic bilateral-symmetry test, which is precisely why the clinician recorded it separately. Still LOOK before acting: the same posture can be voluntary.

A motion line may end with facial observations (eyes deviated to one side, jaw held open, rapid eyelid movement, one side of the face involved, eyes open and unblinking). These come from the face tracker and are ADDED CONTEXT for what you are looking at, never a finding on their own — this population's baseline facial movement is atypical enough that any one of them alone means little.
</perception>
