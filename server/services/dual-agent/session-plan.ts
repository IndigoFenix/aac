...
// generated by five parallel LLM calls.
//
...
//     and the part of day. Cheapest to regenerate. Its two calls split by
//     purpose: `goals` writes prose instructions for the Speaker, `seeds`
//     writes the Board Manager's topic bank (see conversation_seeds).
//
...
const SECTION_RESERVED_XML_TAGS = [
  "persona", "session_goals", "conversation_seeds", "memory", "security", "student_safety",
...
export interface PlanCallSpec {
  call: "identity_core" | "identity_examples" | "situations" | "goals" | "seeds";
  group: PlanGroupKey;
...
export const PLAN_CALLS: ReadonlyArray<PlanCallSpec> = [
  ...
  {
    // The Board Manager's topic bank. Its own call rather than another tag
    // on `goals` because the two compete for output budget and seeds need
    // room to carry a second move each; same GROUP, so it caches and
    // regenerates on the same today-shaped signature.
    call: "seeds",
    group: "goals",
    tags: [{ tag: "conversation_seeds", key: "conversationSeeds" }],
    maxTokens: 2048,
  },
];
...
${ctx.isChild ? authorityDeferenceSpec(ctx) : ""}${closeTag("persona")}`;
    ...
    case "conversation_seeds":
      return `${openTag("conversation_seeds")}
[8–12 seeds. A bank of SPECIFIC things this user could talk about, for the Board Manager to draw on when the conversation flattens out. Write them in ${languageName}.]

WHY THIS EXISTS. When a conversation goes generic the ${T.board} fills with speech acts that have no subject — "I want to talk about something else", "I want to ask a question", "I want to tell you something". None of those can be answered, so the exchange dies one press later. Your job is to make sure the AI always has somewhere specific to go.

Each seed is ONE line:
  - <topic name>: "<opener>" / "<opener>" / "<opener>" → <what the board offers once one is pressed>

The openers are first-person things the USER says, short enough to be ${T.button} text. The part after → is the SECOND MOVE: a topic that dies after one press is barely better than no topic at all.

WHAT MAKES A GOOD SEED:
- ANSWERABLE WITHOUT NEWS. "How was your day" needs the user to have something to report, and to be able to reach it. A preference, a judgment, a ranking, a would-you-rather or a silly hypothetical can be answered by anyone on any day, in one press, and cannot be answered "fine".
- ONE STEP SIDEWAYS FROM AN INTEREST. If the user loves trains, "do you like trains?" is a dead end — they already told us. "Would you rather drive the train or sell the tickets?" is not. Take each interest one step off the obvious.
- SOME SEEDS LET THE USER ASK. Nearly every ${T.button} we build hands them an answer. Buttons that let them interrogate the person in front of them ("what did YOU do today?", "tell me about your dog") move the work onto the fluent speaker and buy the user a whole turn of listening.
- YESTERDAY AND NEXT WEEK, not just today. The schedule above runs from yesterday through the next three days; an event that already happened or is coming up is inherently specific.
- PICK UP A LOOSE THREAD. If the previous-session summary shows something the user was into and did not finish, make one seed that resumes it by name.
- CONCRETE PEOPLE, PLACES AND THINGS BY NAME. Every seed must contain at least one noun a stranger could point at. A seed whose openers name nothing is the exact failure this block exists to prevent.

CONSTRAINTS:
- Age and register: match the user. Do not infantilize an adult, and do not hand a 5-year-old an abstract debate.
- Safety notes win. Never seed a topic the safety notes flag, and never seed a person the user is in conflict with as a light conversational subject.
- Nothing here is a fact claim. A seed proposes what to TALK about — it must never assert that something happened.
- Keep them varied: no two seeds on the same interest, and spread across the kinds above.

NEVER LEAVE THIS SECTION EMPTY. Unlike the other sections, thin data is not a reason to skip it — a user with no notes and no calendar is the one MOST likely to stall. With little to go on, lean on preference and hypothetical seeds pitched at the user's age, which need no history at all.
${closeTag("conversation_seeds")}`;
...
export function buildPlanCall(
  ...
  const needsSituationContext =
    spec.call === "situations" || spec.call === "goals" || spec.call === "seeds";
  // Seeds ARE button copy in the making, so they obey the user's language
  // level; `goals` is prose instructions to the AI and does not.
...
The output you produce is concatenated into the system prompts of the live AI agents that support the user in real time through ${T.board}s. Your sections steer the AI's personality, session aims, gesture interpretations, examples, and safety awareness.`);
  ...
  if (needsSituationContext) {
    ...
    if ((spec.call === "goals" || spec.call === "seeds") && ctx.locationSection) {
      parts.push(ctx.locationSection.trim());
    }
    if ((spec.call === "goals" || spec.call === "seeds") && ctx.lastSessionSummary) {
