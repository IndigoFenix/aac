// server/services/chat/guided-flow/engine.ts
//
// Pure, DB-free step engine. Everything it knows comes from the definition
// and the ctx the caller resolved; it never reads a database, a clock or an
// env var, so it is trivially testable and byte-deterministic.
//
...
import type { GuidedSetupStepStatus } from "@shared/guided-setup";
import type {
  GuidedFlowActionInput,
  GuidedFlowActionResult,
  GuidedFlowDefinition,
  GuidedFlowRecordLike,
  GuidedFlowStepView,
  GuidedFlowView,
  GuidedStep,
} from "./types.js";
...
function skippedSet(record: GuidedFlowRecordLike | null): Set<string> {
 ...
 */
export function resolveFlowView<Ctx, StepId extends string, R extends GuidedFlowRecordLike>(
  def: GuidedFlowDefinition<Ctx, StepId>,
  ctx: Ctx,
  record: R | null,
): GuidedFlowView<StepId> {
  ...
  const steps: GuidedFlowStepView<StepId>[] = def.steps.map((step, i) => {
...
function findStep<Ctx, StepId extends string>(
  def: GuidedFlowDefinition<Ctx, StepId>,
  id: StepId | "done",
): GuidedStep<Ctx, StepId> | undefined {
 ...
 */
export function applyAction<Ctx, StepId extends string, R extends GuidedFlowRecordLike>(
  def: GuidedFlowDefinition<Ctx, StepId>,
  ctx: Ctx,
  record: R | null,
  input: GuidedFlowActionInput<StepId>,
): GuidedFlowActionResult<StepId, R> {
  const view = resolveFlowView(def, ctx, record);

  const refuse = (reason: string): GuidedFlowActionResult<StepId, R> => ({
    record,
  ...
  switch (input.action) {
    ...
    case "skip": {
      ...
      if (record.skipped.includes(targetId)) {
 ...
 */
export function stepPosition<StepId extends string>(view: GuidedFlowView<StepId>): string {
