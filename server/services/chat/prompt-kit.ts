...
import { guidedSetupTool } from "./guided-flow/tool";
...
export interface NlpSchema {
 ...
 export interface formValues {
  ...
  export function buildPromptAndTools(ctx: {
    ...
    /** Offer the `guidedSetup` host tool (flow control only — no data CRUD). */
    ...
    timezone?: string;
    /**
     * Free text appended at the very END of the system prompt, after every
     * other section. Volatile per-turn text belongs here and nowhere else:
     * anything earlier changes the cached Anthropic prefix and turns cache
     * READS (0.1x) into WRITES (1.25x). Used by the Guided Setup step block.
     */
            ...
            function: {
                ...
                parameters: {
                    ...
                    properties: {
                        feature: {
                            type: 'string',
                            enum: ['boards', 'students', 'studentInfo', 'contacts', 'institute', 'progress', 'reports', 'aacsettings', 'interpret', 'customApps'],
                            description: 'The panel to navigate to. boards = AAC board editor, students = student list, studentInfo = the selected student\'s details, contacts = the student\'s contacts/team, institute = institute management, progress = student treatment programs, reports = student reports, aacsettings = the student\'s AAC settings, interpret = communication interpretation, customApps = games & custom apps (play/assign generated quest games)'
                        }
                    },
                    required: ['feature'],
                    additionalProperties: false
    ...
    if (ctx.guidedSetupEnabled) {
        tools.push(guidedSetupTool);
    }
    ...
    // LAST section of the system prompt — see the trailingSection doc above.
    // Nothing may be appended to endPrompt after this point.
    if (ctx.trailingSection) {
        endPrompt += `\n\n${ctx.trailingSection}`;
    }
