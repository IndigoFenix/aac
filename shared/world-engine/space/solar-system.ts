...
export interface GenerateSystemOpts {
  ...
  /** PLANET COMPRESSION (space-time-compression.md §5): a miniature system.
   *  Body radii ÷ s and masses ÷ s² (surface gravity preserved — see
   *  CreateBodyOpts.compression); orbit semi-major axes ÷ s, so the whole
   *  system shrinks uniformly and periods shorten by √s (Kepler over the
   *  scaled gm). Default 1 (real scale). */
  compression?: number;
}
...
export function generateSolarSystem(opts: GenerateSystemOpts): SolarSystem {
  ...
  for (const rb of ordered) {
    ...
    if (parentCB) {
      const aMeters = (pb.formationSemiMajor * AU_M) / compression;
    ...
    const { body } = createCelestialBody({
      ...
      faceN: opts.faceN, bakeGeography: opts.bakeGeography, compression,
    });
