...
export const worldgenSubstrate: SystemSpec = {
  ...
  vars: [
    ...
    // `river`, and feeding `ground` back into the drainage potential would
    // cost the world its rest. `valley` only ever deepens, so a re-routed
    // river leaves a dry bed behind. Both FLOAT, unlike every other var
    // here: nothing drains over them, so they needn't pay the integer
    // grid's termination tax and can carry a sub-unit profile. Above the
    // render/walk line read `ground`; below it (drainage, climate,
    // treeline, sea) read `height`.
    { name: 'valley', min: 0, max: 63, initial: 0, init: 'flat' },
    { name: 'ground', min: 0, max: 63, initial: 0, init: 'flat' },
