...
export interface TownStreets {
  ...
  slots: LotSlot[];
  /** The arterial bearings this tree was grown with (the GrowOpts input,
   *  echoed) — a later PREFIX-CONSISTENT regrow (founding enumeration:
   *  more slots for a founded building) must pass the same bearings. */
  bearings: number[];
}
...
/** Town edge: streets stop here. */
const R_MAX = TOWN_DIMS.townRMax;
...
export function growStreets(seed: number, key: string, minSlots: number, opts?: GrowOpts): TownStreets {
  ...
  return { plazaR: PLAZA_R, streets: st.streets, slots: st.slots, bearings: [...(opts?.bearings ?? [])] };
}
