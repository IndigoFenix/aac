...
/** What a work type contributes to its quarter's character — read off a
 *  compiled economy's work registry; null for structural works (hall,
 *  market) and unknown types. A FREE string (economy.ts BuildingDef.district
 *  widened for zoning ③) — "farm"/"mining"/"craft" are the base worlds'
 *  three; an unrecognized class files as a generic quarter. The standard
 *  classifier over a game's default registry lives with that game's content
 *  (grand-dream city-districts.ts `defaultWorkClass`). */
export type WorkClass = string | null;
...
function kindOf(
  source: FoodSource, works: number[], plan: TownPlan,
  classOf: (type: string) => WorkClass,
): DistrictKind {
  ...
  for (const wi of works) {
    const cls = classOf(plan.works[wi]?.type ?? "");
    if (cls === "farm") farms++;
    else if (cls === "mining") mines++;
    else if (cls === "craft") craft++;
  }
...
export function deriveDistricts(
  ...
  fair: number,
  classOf: (type: string) => WorkClass,
  producerTypes: ReadonlyArray<TownWork["type"]> = ["farm", "hall"],
): CityDistrict[] {
  const populated = catchments.filter(c => c.houses.length > 0);
  if (populated.length === 0) return [];

  // Producers: where this good's supply sets out from (food: farm gates
  // grow it, the hall lands the imports). Supply distance = street
  // meters from the nearest producer to the source.
  const producers: Array<{ x: number; y: number }> = [];
  for (const wk of plan.works) {
    if (producerTypes.includes(wk.type)) producers.push(workDoor(wk));
  }
  ...
  // Assign every production work (anything with a district class) to
  // the district whose source is street-nearest to it — the pithead
  // belongs to the quarter around it.
  plan.works.forEach((wk, wi) => {
    if (classOf(wk.type) === null) return;
