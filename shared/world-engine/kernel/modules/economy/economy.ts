...
export interface BuildingDef {
   ...
   *  A FREE string — content defines the vocabulary ("farm"/"mining"/
   *  "craft" are the base worlds' three); null = no district. */
...
export interface EconomyDoc {
  ...
  species?: SpeciesDef[];
  /** THE NUMERAIRE (nations P3/E4 — money as content): the commodity key
   *  quotes route through once the trade network is dense enough
   *  (kernel/town/money.ts). Currency is the creature-level favor debt
   *  made visible, concrete, and universal within a culture that accepts
   *  the medium — the barter pair-worth math is unchanged, prices are
   *  simply worth expressed against this one good. Absent = pure barter
   *  forever (a village never forced onto coin). Cross-doc: later wins
   *  (the override law). */
  numeraire?: string;
}
...
export interface CompiledEconomy {
  ...
  /** THE NUMERAIRE commodity (E4 — money as content), or null: quotes
   *  route through it once the trade network is dense enough
   *  (kernel/town/money.ts decides WHEN; this only says WHAT). */
...
function mergeDocs(docs: EconomyDoc[]): Required<EconomyDoc> {
  ...
  return {
    ...
    // Numeraire: LATER DOC WINS (the cross-doc override law); "" = none.
    numeraire: docs.reduce<string>((acc, d) => d.numeraire ?? acc, ""),
  };
}
...
function validateDocs(docs: EconomyDoc[], doc: Required<EconomyDoc>): void {
  ...
  // The numeraire must be a real commodity — a typo'd medium of exchange
  // fails naming itself, never a silent barter-forever.
  if (doc.numeraire && !commodities.has(doc.numeraire)) {
    fail(`numeraire "${doc.numeraire}" is not a declared commodity`);
  }
