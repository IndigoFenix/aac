 ...
 *  fetch a unit through the acquire branches and USE it where you stand — the
 *  eat shape minus the station and minus the consumption. This is how an
 *  AFFORDANCE-driven need is served (play with a toy, read a book): the thing
 *  used carries the function, so it is taken out and used ANYWHERE, and it
 *  survives the using — the tidy chore is what eventually puts it back. */
...
export function decideNeed(tpl: NeedTemplate, ctx: NeedCtx): NeedIntent {
  ...
  // Use: equip's shape, but the unit stays in hand and is USED where you are —
  // a toy is played with wherever its owner happens to be standing, not at a
  // designated play station. Empty-handed, fetch one first. The dwell reuses
  // `restHere` so the host needs no new arrival branch; what makes it PLAY
  // rather than dozing is the toy in hand.
  if (tpl.satisfy.kind === "use") {
    if (ctx.carried > 0) return { kind: "restHere" };
    const got = acquireFrom(tpl, ctx);
    if (!got) return { kind: "blocked" };
    // A toy is fetched ONE at a time whatever the branch — you play with a
    // ball, you don't buy six. `want` 1 with no restock target keeps it there.
...
// ── Cooking (round 7 — household-duties-and-sims-mode.md's "(c) COOKING
// over the same transform shape"): the meal chain, the laundry chain's
// mirror but DEMAND-paced. Two data rows over the existing shapes:
//
//   cook   fires while the TABLE holds fewer than `below` MEALS (a stock
//          drive measured in the OUTPUT category via `of` — a meal still
//          in hand counts, which is the loop's brake), takes ONE raw unit
//          (pantry first, else a market buy) and processes it at the
//          OVEN — the transform ADDS the `hot` facet, so the unit leaves
//          the raw category and this row stops firing on it: the type
//          change hands off to serve, exactly the wash → stow seam.
//   serve  the put-it-away row for MEALS: fires while carrying any (the
//          just-cooked unit, a player gift) or while meals lie loose, and
//          sets them ON the table — where a hungry housemate's own walker
...
// pot). serve deposits MEAL units, which nothing acquires (hunger eats
// them from the table's stock without carrying) — no cycle.

/** Cook: keep meals waiting on the table — take raw food (home box, else
 *  a store) and turn it hot at the oven. Give this row to a member who
 *  does NOT shop for food (its priority beats provision, and a transform
 *  fires on ANY matching carried unit — on the food shopper it would
 *  hijack the grocery haul into the pot). */
export function cookTemplate(goodKey: string, mealKey: string, below: number): NeedTemplate {
  return {
    key: `cook:${goodKey}`,
    item: { category: goodKey },
    drive: { kind: "stock", container: "serve", below, of: mealKey },
...
/** Serve: carried or loose meals go ON the table (visible — a table
 *  shows its contents), where the household's hunger finds them waiting. */
export function serveTemplate(mealKey: string, capUnits: number): NeedTemplate {
  return {
    key: `serve:${mealKey}`,
    item: { category: mealKey },
    drive: { kind: "mess", above: 0 },
    satisfy: { kind: "deposit", container: "serve", upTo: capUnits },
    acquire: [{ kind: "loose" }],
    priority: 2.8,
  };
}
