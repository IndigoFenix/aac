...
export interface PlanetLodOpts {
  ...
  seaClamp?: boolean;
  /** Max SUBDIVISIONS per update() call (each builds 4 chunk geometries, a
   *  few ms of synchronous heightAt sampling each). A live host caps this so
   *  a descending camera deepens the tree a step per frame instead of
   *  building dozens of chunks in one frame (the flying-lag spike); the tree
   *  converges over a handful of frames. Default: unlimited (headless tests
   *  and single-shot updates see the full tree immediately). */
  buildBudget?: number;
}
...
export function createPlanetLod(surface: PlanetSurface, host: PlanetLodHost, opts: PlanetLodOpts = {}): PlanetLod {
  ...
  const makeNode = (face: number, uMin: number, uMax: number, vMin: number, vMax: number, depth: number): Node => {
    // NOTE (probed and REVERTED): scaling the skirt with chunk size (10% of
    // edge) to cover LOD cracks builds tens-of-km pale WALLS on coarse chunks
    // — worse than the cracks. The fixed depth stays; the real crack fix is
    // T-junction edge welding between depths.
  ...
  // Subdivisions remaining this update() — see PlanetLodOpts.buildBudget.
  let subdivideBudget = Infinity;

  const updateNode = (node: Node, cam: Vec3): void => {
    const distC = Math.hypot(cam[0] - node.center[0], cam[1] - node.center[1], cam[2] - node.center[2]);
    // Distance to the chunk's NEAREST POINT (bounding sphere), not its center.
    // A coarse chunk is continent-sized: its CENTER sits thousands of km away
    // even when its near edge is right under the camera, so the center metric
    // froze the tree at depth ~1 near the ground — giant facets metres…
    // hundreds of metres off the true height (the "seam" plates the aim probe
    // caught: chunk_5/8/10 at −580…−1950 m datum error 6–17 km from the
    // camera). Nearest-point distance lets refinement cascade along the
    // ground; far chunks (dist ≫ size) behave as before.
    const dist = Math.max(1e-9, distC - node.size);
    ...
    if (shouldSubdivide) {
      if (!node.children) {
        if (subdivideBudget <= 0) {
          // Out of build budget this frame — stay coarse, deepen next frame.
