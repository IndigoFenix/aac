...
/** Stable colour from a participant id, as a THREE colour — derived from the
 *  shared peer palette (peer-colors.ts colorHexForId), so a participant is the
 *  same colour in the 2D view, the 3D view, and a client's video-tile border. */
export function colorForId(id: string): THREE.Color {
  return new THREE.Color(colorHexForId(id));
}
...
export class World3DRenderer {
  ...
  private updateCamera(state: WorldState, dt: number, intent?: RenderIntent): void {
    ...
    if (this.spiritCamera) {
      ...
      // a gaze in a screen CORNER rotates the camera around the house; the screen
      // center is a dead-zone so a resting gaze just interacts. Which corner picks
      // the direction — see spirit/corner-orbit.ts, the law every framed target
      // shares.
      const step = Math.max(0, dt);
      const lookY = FLOOR_HEIGHT * 0.55;
      ...
      // and hand it to the ONE turntable law (spirit/corner-orbit.ts) — the same
      // corner rule the spirit ladder orbits a district/city/region with.
      const gaze = intent?.cursor?.point ?? null;
      if (gaze) {
        const v = this._camScratch.set(gaze.x, lookY + gy, gaze.y).project(this.camera);
        this.spiritAz += cornerOrbitDelta(v.x, v.y, step);
      }
