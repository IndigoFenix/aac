...
export interface TownGoods {
  ...
  /** Units in the house's box of this good at time t (0..boxCap). */
...
/** The household FOOD BOX (pantry crate) center — inside the house, in
 *  its southwest corner. The renderer draws the crate here AND a
 *  returning shopper's trip ends here (throughDoor appends it), so the
 *  box fills exactly where — and when — the body actually stops. */
export function pantryBoxAt(
  center: { x: number; y: number },
  h: { dx: number; dy: number; h: number },
): { x: number; y: number } {
  return { x: center.x + h.dx + 1.75, y: center.y + h.dy + h.h - 1.75 };
}

/** A good's house box corner by SLOT (registration order): 0 SW (the
 *  pantry), 1 SE, 2 NE, 3 NW — four goods before corners run out, and
 *  each good's returning shopper ends at their own crate. */
export function goodBoxAt(
  center: { x: number; y: number },
  h: { dx: number; dy: number; w: number; h: number },
  slot: number,
): { x: number; y: number } {
  const west = center.x + h.dx + 1.75;
  const east = center.x + h.dx + h.w - 1.75;
  const north = center.y + h.dy + 1.75;
  const south = center.y + h.dy + h.h - 1.75;
  switch (slot % 4) {
...
export function createTownGoods(
  ...
  const cycleOf = (house: TownHouse): { period: number; walk: number; trip: number; offset: number } => {
    ...
    const period = Math.max(trip * 1.5, good.capDays * FOOD_DAY_SEC * qfillOf(house));
    const offset = (hashSeed(seed, `${key}:${good.key}:${house.index}`) / 4294967296) * period;
  ...
  const pantry = (house: TownHouse, t: number): number => {
    ...
    return fillOf(house) * boxCap * (1 - homeFrac);
  };
