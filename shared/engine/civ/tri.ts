...
import {
  ...
  type CellGrid, type FoundingSite, type FoundingOpts, type SystemSpec, type TopologySpec,
} from "../cells/index";
...
export function prepareSubstrateOn(opts: PrepareOnOpts): TriPrep {
  const treeline = opts.treeline ?? 40;
  const grid = createGridOn(withRain(opts.spec ?? worldgenSubstrate, opts.rain ?? 1), opts.topology);
  const n = grid.topo.n;
  for (let c = 0; c < n; c++) {
...
export async function foundTri(prep: TriPrep, opts: FoundTriOpts, boot: CompositionBoot): Promise<TriWorld> {
  ...
  const charter = (cell: number): TriCharter => ({
    ...
    timberland: boxSum(grid, "plant", cell, charterR),
  });
