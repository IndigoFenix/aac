...
import { createWorld, stepWorld, worldFastForward, injectEdge, addEntity, setEntityDisabled, type EntityWorld } from "../cells/entities";
...
export async function bootDual(spec: DualSpec, seed: number, boot: CompositionBoot): Promise<DualWorld> {
  ...
  // Conquest state: per-edge SIGNED siege day counters (+n = `a`'s civ
  // attacking for n days, −n = `b`'s; direction change resets) and
  // per-site fractional skirmish-death carries. Grown on founding.
  ...
  async function advanceDay(): Promise<void> {
    ...
    // 3¾. Conquest (§2d): skirmish attrition, siege clocks, resolutions —
    //     the day's war dead land in the same write-back as its vitals.
    processConquest();
  ...
  const isResting = (): boolean =>
    restWorld.isCompositionAtRest() && settlementInert() && !vitalsActive() && !conquestPending();
  ...
  async function foundSettlement(def: FoundSettlementDef): Promise<number> {
    ...
    const grownSiege = new Int32Array(edges.length);
    grownSiege.set(siegeCount);
    siegeCount = grownSiege;
  ...
  function absorbSettlement(loserKey: string, winnerKey: string): boolean {
    ...
    for (const e of ew.adj[li]) { carry[e] = 0; siegeCount[e] = 0; }
  ...
  // Resolved conquests, oldest first — the war ledger.
  const conquestLog: Array<{
    day: number; edge: number; loser: string; winner: string;
    civ: string; mode: "political" | "physical"; casualties: number;
  }> = [];

  /** CONQUEST processing (§2d), one pass per day: skirmish attrition on
   *  hot borders, siege counters, and resolutions. Runs between vitals
   *  and the write-back, so the day's casualties land in the same sync. */
  const processConquest = (): void => {
    const cq = coupling.conquest;
    if (!cq) return;
    const host = ew.edgeAttr[cq.hostilityAttr];
    if (!host) throw new Error(`dual: settlement spec has no edge var '${cq.hostilityAttr}'`);
    const hostAt = cq.hostilityAt ?? 1;
    const majority = nodes.map(n => civOf(n.key)?.trait ?? null);

    // Civ strength: Σ weighted scalars over each civ's majority sites.
    const strength = new Map<string, number>();
    for (let i = 0; i < nodes.length; i++) {
      const m = majority[i];
      if (!m || ew.disabled[i]) continue;
      let s = 0;
      for (const w of cq.strengthScalars) s += w.weight * (ew.scalars[w.scalar]?.[i] ?? 0);
      strength.set(m, (strength.get(m) ?? 0) + s);
    }

    for (let e = 0; e < edges.length; e++) {
      const ia = nodeIdx.get(edges[e].a)!;
      const ib = nodeIdx.get(edges[e].b)!;
      if (ew.disabled[ia] || ew.disabled[ib]) { siegeCount[e] = 0; continue; }
      const ca = majority[ia];
      const cb = majority[ib];
      if (!ca || !cb || ca === cb) { siegeCount[e] = 0; continue; }

      // Stalemate attrition: a hot border bleeds both endpoints daily —
      // through the vitals mechanism, so the ledger stays exact.
      if (cq.skirmish && host[e] > 0) {
        const popsNow = sitePops();
        for (const [i, key] of [[ia, edges[e].a], [ib, edges[e].b]] as Array<[number, string]>) {
          const d = popsNow[i] * cq.skirmish * host[e] + skirmishCarry[i];
          const deaths = Math.floor(d);
      ...
      const dir = sa > 0 && sa >= sb * cq.ratio ? 1 : sb > 0 && sb >= sa * cq.ratio ? -1 : 0;
      if (host[e] >= hostAt && dir !== 0) {
        siegeCount[e] = Math.sign(siegeCount[e]) === dir ? siegeCount[e] + dir : dir;
        if (Math.abs(siegeCount[e]) >= cq.siegeDays) {
          const wi = dir > 0 ? ia : ib;
          const li = dir > 0 ? ib : ia;
          resolveConquest(e, wi, li);
          siegeCount[e] = 0;
        }
      } else {
        // A broken siege cools the border faster than baseline decay —
        // unmatched neighbors don't grind forever.
        if (siegeCount[e] !== 0 && cq.failedSiegeCooling) {
          injectEdge(ew, e, cq.hostilityAttr, -cq.failedSiegeCooling);
        }
        siegeCount[e] = 0;
      }
    }
  ...
  /** One site falls: casualties both sides, then the §2b mode split —
   *  political flip for a town, physical absorption for a village. */
  const resolveConquest = (e: number, wi: number, li: number): void => {
    const cq = coupling.conquest!;
    ...
    // themselves tomorrow.
    const host = ew.edgeAttr[cq.hostilityAttr];
    injectEdge(ew, e, cq.hostilityAttr, -host[e]);
  ...
  /** Conquest worlds with a LIVE border (different majority civs on any
   *  active edge) never claim rest: sieges tick, skirmishes bleed, and
   *  hostility arms daily. Conservative — a bordered stalemate world
   *  steps forever, by design ("visibly bleeding", §2d). */
  const conquestPending = (): boolean => {
    if (!coupling.conquest) return false;
    const majority = nodes.map(n => civOf(n.key)?.trait ?? null);
    for (let e = 0; e < edges.length; e++) {
      const ia = nodeIdx.get(edges[e].a)!;
      const ib = nodeIdx.get(edges[e].b)!;
