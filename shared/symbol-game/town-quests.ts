// shared/symbol-game/town-quests.ts
//
...
import type { EntityDef, FulfillNode, GoalTreeGame, OvercomeNode } from "@shared/goal-tree/types.js";
...
import type { TownPlan } from "@shared/engine/town/plan.js";
import type { CreatureWorld } from "./creatures.js";
...
export function sampleTownNeeds(
  ...
  return samples.slice(0, Math.max(1, max));
}
...
export function buildTownQuestGame(
  ...
  const questCount = Math.max(1, Math.min(3, Math.floor(opts.questCount ?? 2)));
