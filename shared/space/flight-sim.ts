...
export function createPlayer(world: PlayerWorld): Player {
  ...
  function updateAirborne(input: Input, dt: number, ctx: GravityContext) {
    ...
    const altSpeed = nearestAlt > 0 && Number.isFinite(nearestAlt)
      ...
      : 0;
    // MOUSE-WHEEL throttle (the addition on top of seagull's gaze-hyperdrive):
    // an exponential factor the pilot dials to fine-tune cruise speed across
    // the enormous interplanetary/interstellar range. It scales the altitude-
    // target base, so it multiplies through both the hyperdrive and the lock
    // brake below.
    if (input.wheel) {
    ...
    const targetSpeed = Math.max(SPEED.MIN_FLIGHT_SPEED, altSpeed) * state.wheelFactor;
